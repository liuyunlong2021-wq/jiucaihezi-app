...
if (!existsSync(distDir)) {
  throw new Error(`[web-dist] ${distDir} does not exist. Run vite build first.`)
}

const failures = []
for (const requiredFile of requiredFiles) {
  if (!existsSync(resolve(distDir, requiredFile))) {
    failures.push({ path: requiredFile, reason: 'required web file is missing' })
  }
}
