...
13. **[[开发/v1.3.0发布失败修复SDD]]**、**[[开发/v1.3.0遗留测试对齐SDD]]** — v1.3.0 发布前两批过期测试已分别对齐：画布测试双桩适配 `ProjectFileService` 与 V3 asset，Workspace 断言适配设置承接，创作测试适配通用 `rh-aiapp` 动态节点。完整自动门禁已重新通过；Pages 正式部署必须显式带 `--branch=main`，否则只是 Preview。
