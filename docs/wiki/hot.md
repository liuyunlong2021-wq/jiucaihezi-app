...
4. **[[开发/创模式原生附件直连合同SDD]]** — 原生附件合同已实现并完成二次审计：图片走 `image_url`，视频/音频/文件走 `file.file_data`；同 Provider/K 的 Gemini 可在首次知情确认后补位，缺失时只走用户授权的本地工具或明确失败。嵌套媒体计划不保存 Base64，文件提取失败不拦原件，Web 失败保留附件，本地模型零云请求，自定义 Provider 不再冒用默认 K，旧图片入口服从统一能力判断。自动测试、真实同 K 极小媒体请求及 Web/Desktop 构建通过，双端 UI、刷新恢复和三平台安装包人工矩阵待验收。
...
> 上次刷新: 2026-07-21（创模式原生附件合同已完成二次审计修正；双端 UI、刷新恢复与三平台安装包人工矩阵待验收。）
