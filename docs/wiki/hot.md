...
9. **[[开发/电商工作台SDD]]** — 商品图与反推提示词请求是独立单次模型调用，不连接 Chat、创模式或 OpenCode 会话；输入为图片、用户诉求和指定 Skill，输出为一条最终中文提示词与工作台历史。不再提供用途/平台字段猜比例；比例、模型等只在公共 `MediaPlanCard` 中按模型能力由用户确认。确认后，商品图与参考图统一经 `MediaPlanCard -> CreationPanel -> mediaTaskStore` 提交、轮询、落盘和画布展示。Web 与 Desktop 均显示入口，窄屏 Web 也有移动入口；真实 Web 模型和媒体付费链路待人工验收。
...
18. **[[开发/三项生产故障闭环SDD-2026-07-21]]** — RH 明确失败终态已修：`FAILED + errorCode/errorMessage` 不再被可恢复兜底写回 `pending`，刷新恢复会停止轮询并显示上游失败原因；临时轮询错误仍可恢复。专项 75/75、TypeScript、Web 构建和产物审计通过；Web 发布及真实 `1501` 刷新恢复待人工验收。Word 转换已完成生产闭环，画布修复仍待发布验收。
...
20. **[[开发/通用记忆对话独立App SDD]]** — Mac 私测增加逐次发送的 `快速 | 记忆`：两者共用同一 Raw，快速不加载 Wiki/MCP 工具，记忆保留强制查询；上传或粘贴图片已接入公共参考素材合同，确认卡可进入图生图/图生视频。文本模型选择器不再暴露 RunningHub，触发框自适应当前模型并与浮层左对齐；文件树和文档预览保持独立鼠标滚动。绑定普通文件夹仍只检测，点击“新建记忆空间”才创建 Wiki/Raw。RH 3D 第 4-5 项仍待新版 rh-adapter 与 NewAPI 配置。

当前状态：手动“继续写”已从文、武、创全部模式移除；创模式仅保留首次最终正文 SSE 断流的自动续写。

---

> 上次刷新: 2026-07-25（记忆工作台已接通快速/记忆逐轮模式、粘贴图片附件和公共媒体参考素材桥接；Skill 收敛为单入口向上浮层，附件改为紧凑标签，文本模型按家族分组且隐藏 RunningHub 媒体模型，触发框自适应并与浮层左对齐；文件树与文档预览已恢复独立滚动。）
