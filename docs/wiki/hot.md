...
20. **[[开发/通用记忆对话独立App SDD]]** — `v2.0.1` Web 已发布。记忆工作台上传附件现按需写入 `jc-media/uploads/`，Raw 保存稳定项目路径；每轮从当前对话重建受控历史素材引用，确认卡刷新、隔轮复用和换模型后均重新读取同一项目文件，不再依赖首次上传页面内存。媒体链路继续区分生成、结果取回和项目保存，图片支持一次 `1-5` 个独立付费任务。

当前状态：手动“继续写”已从文、武、创全部模式移除；创模式仅保留首次最终正文 SSE 断流的自动续写。

---

> 上次刷新: 2026-07-26（补齐记忆工作台项目级附件持久化和历史媒体引用；Raw 不保存二进制。）
