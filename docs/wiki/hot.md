...
20. **[[开发/通用记忆对话独立App SDD]]** — Mac 私测增加逐次发送的 `快速 | 记忆`：两者共用同一 Raw，快速不加载 Wiki/MCP 工具，记忆保留强制查询；上传或粘贴图片已接入公共参考素材合同，确认卡可进入图生图/图生视频。绑定普通文件夹仍只检测，点击“新建记忆空间”才创建 Wiki/Raw。RH 3D 第 4-5 项仍待新版 rh-adapter 与 NewAPI 配置。

当前状态：手动“继续写”已从文、武、创全部模式移除；创模式仅保留首次最终正文 SSE 断流的自动续写。

---

> 上次刷新: 2026-07-25（记忆工作台已接通快速/记忆逐轮模式、粘贴图片附件和公共媒体参考素材桥接；Skill 收敛为单入口向上浮层，附件改为紧凑标签，顶部模型改为按家族分组的固定浮层；两种模式共用 Raw，快速不强制 Wiki，图片计划不再静默降级为文生图。）
