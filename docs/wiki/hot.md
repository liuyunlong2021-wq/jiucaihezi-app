...
20. **[[开发/通用记忆对话独立App SDD]]** — `v2.0.1` Web 已发布，本轮准备发布顶部职责重组与批量媒体确认修复。SDD 已固化八条实测经验：文件单一真源、逐轮快速/记忆、对话与资料导航分工、AI 输出防御性解析、素材引用物化、明确失败终态、视觉主基线、构建/预览/Production 证据分层。媒体计划接受单对象和对象数组，批量任务逐项显示独立确认卡；解析成功后不裸显技术 JSON，非法计划保留原文排障。

当前状态：手动“继续写”已从文、武、创全部模式移除；创模式仅保留首次最终正文 SSE 断流的自动续写。

---

> 上次刷新: 2026-07-26（记忆工作台顶部主分界线已贯通，Logo 完成光学对齐，文件动作与搜索降为线下次级层。）
