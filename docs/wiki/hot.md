...
20. **[[开发/通用记忆对话独立App SDD]]** — 第四阶段 4A 云项目入口已完成：左上项目中心统一展示本机/云端项目，本地项目可首次上传，云项目可下载为 Web 或 Mac 空本地副本。绑定后文本写入自动推送，项目打开/App 前台/网络恢复自动先拉后推；“立即同步”只用于强制执行和失败重试。设置页不再承担 owner 配对。

当前状态：手动“继续写”已从文、武、创全部模式移除；创模式仅保留首次最终正文 SSE 断流的自动续写。

---

> 上次刷新: 2026-07-27（云项目产品入口完成，进入 v2.0.4 发布门禁。）
