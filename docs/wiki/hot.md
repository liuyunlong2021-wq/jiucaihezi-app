...
**当前焦点：[[开发/通用记忆对话独立App SDD]]** — 4C-1 至 4C-13 已完成，iPhone 分支已合并 `main` 并删除。V2.1.0 Web、桌面三平台和 iOS TestFlight 已通过真实验收；Android 分支 `codex/android-app` 已完成 4D-1 工具链/工程初始化及 4D-2 vivo V2443BA 真机 Debug 安装、冷启动和 60 秒稳定运行验收，4D-3 已接入 Android Keystore 但等待真账号登录/恢复验收。Excel/PPT 转换依赖仍待服务器部署和真实文件验证。
