...
9. **[[开发/电商工作台SDD]]** — 商品图与反推提示词请求是独立单次模型调用，不连接 Chat、创模式或 OpenCode 会话；输入为图片、用户信息和指定 Skill，输出为一条最终中文提示词与工作台历史。确认后，商品图与参考图统一经 `MediaPlanCard -> CreationPanel -> mediaTaskStore` 提交、轮询、落盘和画布展示。定向测试、类型检查和 Desktop 构建已通过；真实模型和媒体付费链路待人工验收。
