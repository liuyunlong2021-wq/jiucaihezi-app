...
13. **[[开发/v1.3.0发布失败修复SDD]]** — v1.3.0 发布前的 focused 测试失败已审计并修复：画布测试双桩适配 `ProjectFileService` 与 V3 asset 结构，关闭确认统一为 `confirmAction`，过期模型/Skill 测试预期已同步。所有自动门禁已通过；Pages 正式部署必须显式带 `--branch=main`，否则只是 Preview。
