...
20. **[[开发/通用记忆对话独立App SDD]]** — Web 媒体稳定性按成熟产品收敛为“任务元数据 -> 项目资产 -> 轻量卡片 -> 单实例按需预览”；记忆输入框新增统一 `@` 引用，动态选择当前项目文件/媒体或用户已安装 Skill，显式 Skill 通过标准 tool call/result 预加载。第四阶段 4A 的 D1 文字同步链路保持不变；`v2.0.3` 同时修复 Apple Silicon 发布签名链。

当前状态：手动“继续写”已从文、武、创全部模式移除；创模式仅保留首次最终正文 SSE 断流的自动续写。

---

> 上次刷新: 2026-07-26（媒体时间线有界渲染与 Apple Silicon 发布稳定性修复进入 v2.0.3 门禁。）
