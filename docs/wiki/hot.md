...
- **结果优先的最小两阶段流程。** ReadPlan 只负责选择最少页面，`paths` 可以为空；入口为空、缺页、读取失败或计划解析失败都继续把真实上下文交给模型。模型第二阶段始终返回用户答案，只有存在 `changePlan` 时才执行一次 Wiki apply。`index.md`、双链、来源、日志和写后验证由程序负责。Wiki 写入结果通过独立程序状态卡显示，不拼接完整内部回执到正文。
...
- **本机 MLX 采用“外部服务、App 只连接”的最小合同（2026-08-23）。** Desktop 设置只显示服务地址（默认 `http://127.0.0.1:8081`）、连接按钮、状态和自动识别的模型数量；用户自行下载模型、配置并启动 MLX 服务，韭菜盒子不读取模型目录、不下载文件、不管理进程。连接只接受 localhost/127.0.0.1/::1，通过 `/v1/models` 保存 `local-mlx` 模型并复用现有 OpenAI/SSE/工具链；简单任务优先快速模式，需要 Wiki/项目工具时使用记忆模式。见 [[开发/通用记忆工作台本机MLX兼容服务接入TDD-2026-08-21#10. 最终界面合同与首次使用流程（2026-08-23）]]。
