...
20. **[[开发/通用记忆对话独立App SDD]]** — `v2.0.1` Web 已发布。顶部轻量对话选择器已直接投影 `.raw/对话记录/`，支持搜索、切换、新建、重命名和删除；记忆文件树默认隐藏 `.raw` 并只负责资料。普通文件已统一进入不销毁当前对话的临时预览层，返回后保留滚动、草稿、附件与引用。定向回归 `21/21`、类型检查、Web quick build和浏览器真实交互通过。媒体结果安全合同保持“可信执行来源 + HTTPS 基础检查 + 原始链接兜底”；Desktop 统一媒体代理留待后续版本。

当前状态：手动“继续写”已从文、武、创全部模式移除；创模式仅保留首次最终正文 SSE 断流的自动续写。

---

> 上次刷新: 2026-07-26（记忆工作台已实施 Raw 对话选择器、资料预览层与文件树职责分离。）
