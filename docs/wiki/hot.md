...
> 更新：2026-08-14 | 阶段：v2.1.22；小易图片主链与官方渠道均已生产验收

## 当前结论

- **小易异步图片完成态超大载荷已本地修复，待生产部署验收。** 生产任务表已确认卡住任务实际均为 `SUCCESS`，适配器最后状态为 `completed`；Desktop 却仍显示 `processing`，根因是上游返回的 `b64_json` 被透传进 NewAPI 轮询响应。适配器现固定请求 URL 结果，避免整张图片反复经过任务状态链；适配器 `8/8` 与前端 focused 已通过，尚未部署，不能写成生产已修复。见 [[运维/小易图片异步适配与部署-2026-08-14]]。
