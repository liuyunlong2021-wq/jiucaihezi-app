...
20. **[[开发/通用记忆对话独立App SDD]]** — `v2.0.0` 已发布。Mac 顶部空白标题栏与标题文字补齐 Tauri 原生拖拽区域；渠道 94 Gemini 图片模型与渠道 82 Veo 3.1/Fast 视频模型已按真实合同启用。`rh-gpt2-official` 仍固定走渠道 61 的 rh-adapter，展示价修正为 `¥0.25`；生产恢复需部署补齐其精确映射的 `creation-models`。

当前状态：手动“继续写”已从文、武、创全部模式移除；创模式仅保留首次最终正文 SSE 断流的自动续写。

---

> 上次刷新: 2026-07-26（`v2.0.0` 已发布；补齐 Mac 拖拽、Gemini/Veo 直连合同及 RH GPT Image 2 可用性映射。）
