...
20. **[[开发/通用记忆对话独立App SDD]]** — Web 与 Mac 2.0.1 已完成，第四阶段先建设同账号文字同步底座再做 Mobile。媒体确认卡新增用户可编辑提示词，用户明确给出的提示词默认原样使用；同名模型按“直连 / RunningHub”消歧。跨设备仍只同步 Raw、Wiki 与安全文本，不同步媒体或凭据。

当前状态：手动“继续写”已从文、武、创全部模式移除；创模式仅保留首次最终正文 SSE 断流的自动续写。

---

> 上次刷新: 2026-07-26（媒体提示词控制权与同名模型消歧；第四阶段仍先做文字同步底座。）
