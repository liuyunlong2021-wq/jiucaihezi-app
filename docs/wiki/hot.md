...
**当前焦点：[[开发/通用记忆对话独立App SDD]]** — 4C-1 至 4C-13 已完成，iPhone 分支已合并 `main` 并删除。V2.1.0 Web、桌面三平台和 iOS TestFlight 已通过真实验收；Android 分支 `codex/android-app` 已完成工具链与 Tauri 工程初始化，aarch64 Debug APK 已构建但无 Android 设备，4D-2 等待真机安装/启动验收。Excel/PPT 转换依赖仍待服务器部署和真实文件验证。
