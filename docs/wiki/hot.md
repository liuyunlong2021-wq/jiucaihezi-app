...
20. **[[开发/通用记忆对话独立App SDD]]** — 记忆模式已改为官方式 Skill 渐进披露，输入框不设 Skill 选择器，快速模式仍不提供工具。两次 Chrome Renderer dump 证明媒体结果卡自动读取 OPFS 全文件并复制解码会触发 V8 OOM；现改为安全 HTTPS 懒加载展示，项目副本只在用户下载、预览或引用时读取，生成落盘和历史素材复用不变。

当前状态：手动“继续写”已从文、武、创全部模式移除；创模式仅保留首次最终正文 SSE 断流的自动续写。

---

> 上次刷新: 2026-07-26（记忆模式 Skill 官方式自动发现；修复媒体结果卡自动预读项目文件导致的 Chrome OOM。）
