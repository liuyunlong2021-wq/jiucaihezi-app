...
**当前焦点：[[开发/通用记忆对话独立App SDD]]** — 4C 已从 `v2.0.9` 建立独立 iPhone 分支并生成 Tauri 官方 Xcode 工程；`rfd` 与 `trash` 已限制为 Desktop，真实 iPhone 与 Simulator Rust target 交叉检查通过。完整 Simulator `.app` 已构建、安装并显示图标，启动受 Tauri 官方未关闭的 #14675 阻塞；真机仍需 Apple Development 证书、Team ID、独立 Mobile Bundle ID 和已连接 iPhone。后续按登录与云项目 → 移动聊天 → 媒体能力 → TestFlight 推进。
...
> 上次刷新: 2026-07-27（iPhone 4C 独立分支、双 iOS target 和 Simulator 安装完成；运行受 Tauri 官方 #14675 阻塞。）
