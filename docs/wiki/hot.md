...
> 更新：2026-08-20 | 阶段：v2.1.30 自动门禁已通过；Codex 创作 MCP 待实施
...
- **Codex 调用韭菜盒子创作能力的 MCP TDD 已建立，尚未实施。** 采用“Codex stdio MCP -> Desktop 本机鉴权桥接 -> 现有 Creation Runtime / `mediaTaskStore`”的单一链路；不复制 `jc_media.py`、静态模型表、API Key、轮询器或历史数据库。Codex 任务必须进入创作面板同一历史并由现有项目落盘与显式画布动作处理。图片可按 MCP 图片内容验收；视频/音频是否在 Codex 内嵌播放必须真实客户端验证，当前只承诺稳定本地资源合同。见 [[开发/韭菜盒子Codex创作MCP服务TDD-2026-08-20]]。
