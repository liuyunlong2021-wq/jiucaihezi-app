...
**当前焦点：[[开发/通用记忆对话独立App SDD]]** — 4C 已建立独立 iPhone 分支；Mobile Bundle ID `com.jiucaihezi.mobile`、Team ID `RXD4L9387J`、Apple Development 证书和真机均已就绪。真实 iPhone 已完成编译、签名、IPA 导出、安装和启动请求，但与 Simulator 一样在业务代码前崩于 `wry::wkwebview::platform_webview_version -> CFRelease(NULL)`，准确命中 Tauri 官方未关闭的 #14675。上游阻断解除前不能宣称 iPhone 壳可运行，也不能开始首页、登录和云项目验收；不以整体降级或本地 Wry fork 掩盖。
...
> 上次刷新: 2026-07-27（iPhone 真机编译、签名、导出和安装完成；真机与 Simulator 均确认运行受 Tauri 官方 #14675 阻塞。）
