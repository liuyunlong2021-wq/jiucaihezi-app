...
20. **[[开发/通用记忆对话独立App SDD]]** — Mac 私测增加逐次发送的 `快速 | 记忆`：两者共用同一 Raw，快速不加载 Wiki/MCP 工具，记忆保留强制查询；上传或粘贴图片已接入公共参考素材合同，确认卡可进入图生图/图生视频。媒体成功后自动落当前项目，Raw 只记 `jc-media/...` 路径，结果卡可下载副本或在文件树定位；生成状态与当前主题同步。文本模型选择器不再暴露 RunningHub，触发框自适应当前模型并与浮层左对齐；文件树和文档预览保持独立鼠标滚动。RH 3D 第 4-5 项仍待新版 rh-adapter 与 NewAPI 配置。
