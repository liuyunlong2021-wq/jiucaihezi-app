...
20. **[[开发/通用记忆对话独立App SDD]]** — Mac 私测增加逐次发送的 `快速 | 记忆`：两者共用同一 Raw，快速不加载 Wiki/MCP 工具，记忆保留强制查询；上传或粘贴图片已接入公共参考素材合同，确认卡可进入图生图/图生视频。媒体成功后自动落当前项目，Raw 只记 `jc-media/...` 路径，结果卡可下载副本或在文件树定位；生成状态与当前主题同步。文本模型选择器不再暴露 RunningHub，触发框自适应当前模型并与浮层左对齐；文件树和文档预览保持独立鼠标滚动。`gpt-image-2-vip` 已接入公共媒体注册、OpenAI 文生图/图生图合同和渠道可用性检测，待发布 APP 与 `creation-models` 服务。

当前状态：手动“继续写”已从文、武、创全部模式移除；创模式仅保留首次最终正文 SSE 断流的自动续写。

---

> 上次刷新: 2026-07-25（`gpt-image-2-vip` 已完成 APP 与渠道可用性服务接入，待发布。）
