...
20. **[[开发/通用记忆对话独立App SDD]]** — `v2.0.1` Web 已发布。媒体链路现已区分生成、受保护结果取回和项目保存：韭菜盒子 API 结果下载携带当前 Token，第三方 CDN 不携带；生成成功但保存失败仍展示结果并可重试保存。确认卡提交后可换模型/比例再次生成，图片支持一次 `1-5` 个独立付费任务并保留全部历史结果。

当前状态：手动“继续写”已从文、武、创全部模式移除；创模式仅保留首次最终正文 SSE 断流的自动续写。

---

> 上次刷新: 2026-07-26（修复 NewAPI 受保护视频结果下载 401；媒体卡支持卡内再生成和图片 1-5 张批量变体。）
