...
> 更新：2026-08-14 | 阶段：v2.1.22；小易图片主链已闭环，官方稳定渠道待验收

## 当前结论

- **`v2.1.22` 小易图片适配器主链已生产闭环，上游账号池存在间歇波动。** 渠道 88 的 1K 已真实成功，此前长时间等待的三参考图任务也最终成功并由 App 落盘，确认 App、NewAPI、适配器、小易异步编辑与轮询链路正常。此前 `No available 1K/2K image accounts` 和 `fetch failed` 应判定为小易上游间歇不稳定。新 `gpt-image-2-官方 -> gpt-image-2` 稳定渠道已加入代码，待适配器重部署和真实生成验收。见 [[运维/小易图片异步适配与部署-2026-08-14]]、[[排障/小易GPT图片多参考图与账号池失败-2026-08-14]]。
