...
20. **[[开发/通用记忆对话独立App SDD]]** — 第四阶段 4A 进行中：APAC 独立 D1 `jiucaihezi_sync` 的远端三表、基础索引和 revision 唯一门禁均已核验；Gateway 已本地实现登录鉴权、账号隔离、项目列表/创建、增量拉取、幂等批量推送及删除/恢复，合同测试与双 D1 dry-run 通过。Gateway 尚未部署，客户端同步队列尚未接入；只同步 Raw、Wiki 与安全文本，不同步媒体或凭据。

当前状态：手动“继续写”已从文、武、创全部模式移除；创模式仅保留首次最终正文 SSE 断流的自动续写。

---

> 上次刷新: 2026-07-26（第四阶段 Gateway 文字同步接口本地实现并验证，尚未部署或接入客户端。）
