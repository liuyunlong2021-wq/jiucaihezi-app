...
20. **[[开发/通用记忆对话独立App SDD]]** — Web 媒体稳定性按成熟产品收敛为“任务元数据 -> 项目资产 -> 轻量卡片 -> 单实例按需预览”：记忆时间线复用现有虚拟列表，历史视频/音频不再常驻播放器；第四阶段 4A 的 D1 文字同步链路保持不变。`v2.0.3` 同时清理 macOS OpenCode sidecar 上游签名，并为 Apple 时间戳服务偶发失败增加一次完整 ARM 构建重试。
