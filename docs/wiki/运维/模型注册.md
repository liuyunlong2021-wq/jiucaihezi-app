...
## 三、RH AI App 注册（统一 `rh-aiapp`）

RunningHub AI App 不是普通 RH 模型：创作面板和 NewAPI 只使用一个通用模型名 `rh-aiapp`，具体工作流通过 `webappId` 选择。不要为每个应用新增 `rh-aiapp-xxx` 模型，也不要把渠道 ID 写入客户端。

### 3.1 本地注册

在 `rh-adapter/src/models/mapping.py` 的 `_ai_app_registrations()` 登记列表中加入目录项（`get_ai_app_directory()` 现在只是这个列表的对外过滤器）：

```python
{
    "webappId": "RunningHub AI App ID",
    "label": "面板显示名称",
    "outputType": "video",  # image / video / audio
    "billingModel": "rh-aiapp",
}
```

节点参数不在代码中静态复制。App 选择应用后请求 `/api/runninghub/app-info?webappId=...`，由 RH Adapter 调 RunningHub `apiCallDemo` 动态发现 `nodeInfoList`。

**出图的应用不要走上面这条**。登记进 `IMAGE_MODELS`（`endpoint: None` + `webapp_id` + `output_type: "image"`）就会自动从「AI 应用」下拉隐藏、改以图片模型出现在创作面板，但提交仍走 `webappId` 工作流（`is_ai_app_model` 的判定条件）。先例：`Qwen-image-2.1`（`2101972248130318338`，2026-09-21）。

### 3.2 服务器白名单与部署

将 `webappId` 加入 `/opt/rh-adapter/.env` 的 `RH_AI_APP_WHITELIST`，并确保 `docker-compose.yml` 有对应的环境变量传递：

```yaml
- RH_AI_APP_WHITELIST=${RH_AI_APP_WHITELIST:-}
```

代码更新必须从仓库目录复制后重建镜像；`restart` 或不带 `--build` 不会更新 Dockerfile 已复制的源码：
...
grep -n "<webappId>" rh-adapter/src/models/mapping.py
cp -r rh-adapter/* /opt/rh-adapter/
cd /opt/rh-adapter
...
```bash
docker exec rh-adapter-rh-adapter-1 \
  grep -n "<webappId>" /app/src/models/mapping.py
curl -s https://api.jiucaihezi.studio/api/runninghub/app-directory
```

容器内 `mapping.py` 和公网目录接口都必须出现新 ID，且目录项的 `billingModel` 必须是 `rh-aiapp`。接口返回正确后刷新 App，应用才会出现在“AI 应用 → 应用”下拉框。

两个容易误判的点（2026-09-21 实测）：

- **只改 `.env` 不会生效**。环境变量在容器启动时注入，必须 `docker compose up -d --force-recreate rh-adapter` 重建容器（会一并清空内存任务表）。
- **白名单非空时漏加 ID 的典型表现**：容器内 `grep /app/src/models/mapping.py` 能看到新 ID，但 `app-directory` 不返回它、创作面板里也找不到 —— 先查 `docker compose exec -T rh-adapter env | grep RH_AI_APP_WHITELIST`。白名单为空 = 不限制，非空 = 只放行列出的 ID；白名单有值却只写新 ID，会把其余应用全挡掉。

### 3.4 已验证示例（2026-08-29）

以下 7 个 Minimax-h3 应用已按上述合同注册并显示：

| 名称 | webappId |
|---|---|
| Minimax-h3 文生视频 | `2093604127250149377` |
| Minimax-h3 首帧图生视频 | `2093571735550521345` |
| Minimax-h3 首尾帧 | `2093579373894000642` |
| Minimax-h3 多参双图 | `2093654136997900290` |
| Minimax-h3 多参-4图 | `2093651661213491202` |
| Minimax-h3 多参3图 | `2093662476146667522` |
| Minimax-h3多参5图 | `2093706819385516034` |

本次失败根因是先在错误目录执行 `cp rh-adapter/*`，导致复制失败但 Docker 仍重建旧源码；修复为在 `/opt/jiucai-repo` 执行复制，并补齐白名单后，目录接口返回 11 项、创作面板显示新增 6 项。
