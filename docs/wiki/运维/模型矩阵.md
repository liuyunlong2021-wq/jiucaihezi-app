...
| 2 | GPT Image 2 VIP · 直连 | `gpt-image-2-vip` | `/v1/images/generations` | none | ✅ auto→`/v1/images/edits` | ✅ |
| 3 | Grok 4.2 Image · T8 直连 | `grok-4.2-image` | `/v1/images/generations` | none | ⚠️ partial | partial |
| 4 | Gemini 3.1 Flash Image · 直连 | `gemini-3.1-flash-image-preview` | `/v1/images/generations` | none | ✅ auto→`/v1/images/edits` | ✅ 渠道 94；¥0.1 |
| 5 | Gemini 3.1 Flash Image 2K · T8 直连 | `gemini-3.1-flash-image-preview-2k` | `/v1/images/generations` | none | ✅ 上传触发 | partial |
| 6 | Gemini 3 Pro Image · 直连 | `gemini-3-pro-image-preview` | `/v1/images/generations` | none | ✅ auto→`/v1/images/edits` | ✅ 渠道 94；¥0.2 |
| 7 | Gemini 3 Pro Image 2K · T8 直连 | `gemini-3-pro-image-preview-2k` | `/v1/images/generations` | none | ✅ 上传触发 | partial |
| 8 | Gemini 3 Pro Image 4K · T8 直连 | `gemini-3-pro-image-preview-4k` | `/v1/images/generations` | none | ✅ 上传触发 | partial |
...
- **渠道 94 Gemini 图生图**: 自动切换 `/v1/images/edits` + multipart form data；同步响应的 `b64_json` 由现有 OpenAI 图片提取器处理
...
| 11 | Veo 3.1 · 直连 | `veo-3.1-generate-preview` | `/v1/videos` | newapi-task | ✅ 渠道 82；文生/图生；¥0.2/秒 |
| 12 | Veo 3.1 Fast · 直连 | `veo-3.1-fast-generate-preview` | `/v1/videos` | newapi-task | ✅ 渠道 82；文生/图生；¥0.1/秒 |

### 关键设计决策

- **directVideo 默认端点**: `/v1/videos`（之前错误使用 `/v1/video/generations`，已修复）
- **渠道 82 Veo 图生视频**: `/v1/videos` multipart，首帧字段为 `input_reference`；完成后读取 `/v1/video/generations/{task_id}` 的 `result_url`，内容由 `/v1/videos/{task_id}/content` 代理下载
