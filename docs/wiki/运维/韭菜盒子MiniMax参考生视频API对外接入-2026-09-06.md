...
>
> 2026-09-18 修订：补充任务数据保留期（7 天）说明，见「下载成片」与「错误处理」。
...
| `images` | 否 | 参考图 URL 数组，**至少 1 张**、最多 9 张。 |
| `audios` | 否 | 参考音频 URL 数组，最多 3 段。 |

### 比例与分辨率

菠萝模型的画幅**完全由 `resolution` 的后缀决定**：

- `16:9` 用 `480p横` 或 `768p横`
- `9:16` 用 `480p竖` 或 `768p竖`
- `1:1` 只有 `minimax_h3_zm_u24` 支持，用 `480p(1:1)` 或 `768p(1:1)`

客户端可以额外传 `ratio` 或 `aspect_ratio`，但它**不会**改写画幅，只在没传 `resolution` 时用来推导默认分辨率（`16:9`→`768p横`、`9:16`→`768p竖`、`1:1`→`768p(1:1)`）。**如果比例和分辨率互相矛盾**（例如 `aspect_ratio: 16:9` 配 `768p竖`），接口直接返回 `400`，不会静默出方向不对的成片。推荐只传 `resolution`。
