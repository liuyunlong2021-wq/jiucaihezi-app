...
| 比例 | 请求里没有 `ratio` / `aspect_ratio` 字段，方向靠 `resolution` 的后缀表达 | **方向完全以 `resolution` 为准**，创作面板也不再展示比例、不发比例字段。适配器只在客户端没给 `resolution` 时用 `aspect_ratio` 推导默认值（`16:9`→`768p横`、`9:16`→`768p竖`、`1:1`→`768p(1:1)`）；两者都给了且互相矛盾则直接 400 |
