...
| 比例 | 请求里没有 `ratio` / `aspect_ratio` 字段，方向靠 `resolution` 的后缀表达 | 面板送 `aspect_ratio`，适配器把它当**权威方向**并按轴重建分辨率名（保住 `480p` / `768p` 档位）：`16:9`→`横`，`9:16`→`竖`，`1:1`→`(1:1)`。所以 `16:9` + `768p(1:1)` 会变成 `768p横`，不会被分辨率带跑；模型不支持该轴时直接 400 |
| 1:1 分辨率 | 只有 `minimax_h3_zm_u24` 和两个 `lightx2v` 模型支持 `480p(1:1)` / `768p(1:1)` | 适配器按模型各自的白名单放行：`zm_u24` 放行 6 个分辨率、面板比例多一个 `1:1`；旧版 `v2_15s` 收到 1:1 就 400 |
