## 接口:/rhart-audio/suno-v5.5/single
copy
suno-single-v5.5
...
```curl
curl --location --request POST 'https://www.runninghub.cn/openapi/v2/rhart-audio/suno-v5.5/single' \
...
| --- | --- | --- | --- |
| `title` | String | 可选 | 歌曲标题；留空时 Suno 会根据内容自动取一个<br>文本长度限制: 0 - 80 |
| `description` | String | 必填 | 用一段文字描述想要的音乐（风格/情绪/场景），不超过 400 字符<br>文本长度限制: 1 - 400 |
| `make_instrumental` | String | 可选 | 是否生成纯演奏音乐（不含人声）；传字符串 "true"/"false"<br>枚举值: [false, true] |
...
## 接口:/rhart-audio/suno-v5.5/custom
copy
suno-custom-v5.5
...
```curl
curl --location --request POST 'https://www.runninghub.cn/openapi/v2/rhart-audio/suno/lyrics' \
--header "Content-Type: application/json" \
--header "Authorization: Bearer ${RUNNINGHUB_API_KEY}" \
--data-raw '{
...
| 参数说明 | 类型 | 必填/可选 | AI 应用程序生成的结果。 |
| --- | --- | --- | --- |
| `prompt` | String | 必填 | 用一句话描述想要的歌词主题（≤500 字符）<br>文本长度限制: 1 - 500 |
