...
| Desktop sidecar 打包 | `tauri.studio.conf.json`；默认记忆 `tauri.conf.json` 明确排除 | ✅ |
...
| `server.ts` 健康检查 | 健康检查 | ✅ |
| `server.ts` `preferAppEnv()` | 应用级缓存的环境变量注入 | ✅ |
...
| OpenCode | 我们的代码 | 状态 |
|----------|-----------|------|
| `renderer/index.tsx` | `main.ts` + Vite `@app-root` 静态产品入口 | ✅ |
| `app/app.tsx` | Studio：`StudioApp.vue` + `WorkspaceLayout.vue` | ✅ |
| 记忆独立产品壳 | `App.vue` + `MemoryWorkbench.vue` | ✅ |
...
| `packages/app/src/context/server-sync.tsx` bootstrap + 目录 sync store | `src/stores/openCodeSyncStore.ts` | ✅ |
...
| `promptAsync({ model, variant })` | `openCodeSyncStore.submitPrompt()` | ✅ |
