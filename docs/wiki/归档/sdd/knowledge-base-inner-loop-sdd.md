...
## 5. 已定决策（用户 2026-06-26 拍板）

| # | 决策点 | 结论 |
|---|--------|------|
| 1 | 建库 UI 入口位置 | **A. 工具仓库新增「知识库模板」区** |
| 2 | 指令库入口形态 | 输入框旁小按钮，文案叫**「指令」**（不叫「💡 知识库怎么用」） |
| 3 | 第一版指令范围 | **只做小说/剧本**，律师等 v2 再加 |
| 4 | 建库是否做成 skill | **要做一个建库 skill**，用本机 skill-creator 创建（见 §5.1）。与模块一「APP 一键建库」并存：APP 建库给固定 3 模板（确定性、零 token）；建库 skill 给非标职业 / 定制需求（AI 访谈生成精细目录）。 |

### 5.1 建库 skill 与 APP 一键建库的分工

```
APP 一键建库（模块一，确定性）
  → 律师/小说/漫剧 3 个固定模板，点一下精确创建，不耗 token
建库 skill（AI 推理，覆盖长尾）
  → 用户是医生/产品经理/研究生等非标职业
  → skill 访谈用户（你做什么？要管哪些信息？）
  → 生成定制精细目录 + CLAUDE.md
  → 可沉淀成新的 APP 模板
```

两者不冲突，是"标准化"与"长尾定制"的互补。
