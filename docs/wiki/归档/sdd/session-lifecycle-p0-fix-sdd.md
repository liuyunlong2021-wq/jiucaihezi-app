...
## 三、修改文件清单（全部完成 · 6 文件 · ~200 行）

| 文件 | 内容 | 状态 |
|------|------|:--:|
| `src/opencodeClient/eventBridge.ts` | SSE 重连循环（250ms + 15s 心跳） | ✅ |
| `src/opencodeClient/session.ts` | listOpenCodeSessions + getOpenCodeSession + promptAsync 修复 | ✅ |
| `src/stores/sessionStore.ts` | mergeOpenCodeSessions + 桌面端跳过 IndexedDB | ✅ |
| `src/stores/agentStore.ts` | boot 时同步 OpenCode 会话 | ✅ |
| `src/components/chat/ChatPanel.vue` | 切换 project → 刷新会话列表 | ✅ |
| `src/components/filetree/FileTreePanel.vue` | Tab 项目在左 + 精简工具栏 | ✅ |
...
- [x] `cargo check` 通过
- [x] `vite build` 通过
- [x] 桌面端: 启动 → SSE 事件正常 → session.idle 完成
- [x] 桌面端: 点击历史会话 → 从 OpenCode API 加载消息 → 继续聊
- [x] 桌面端: 切换 project → 会话列表切换（新项目新会话）
- [x] 桌面端: SSE 心跳可见（server.heartbeat）
- [x] 桌面端: promptAsync 不再阻塞（503 不挡路）
- [x] 桌面端: 本地模型 / 云端模型均正常
- [x] Web 端: 不受影响（`isTauriRuntime()` 守卫）

## 五、附加修复（测试中顺手修）

| 修复 | 文件 | 说明 |
|------|------|------|
| promptAsync 替代 prompt | session.ts | 原用同步 prompt() 等 LLM，503 时阻塞报错；改为 promptAsync() 立即返回 204 |
| 桌面端彻底告别 IndexedDB | sessionStore.ts | `loadAllSessions()` 桌面端 skip，消除双源竞态 |
| 项目 Tab 在左 | FileTreePanel.vue | 跟 VS Code 一致：项目 | 会话 |
| 删除未用变量 | eventBridge.ts | attempt 计数器未读，删 |
