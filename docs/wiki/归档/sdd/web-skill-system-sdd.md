...
| commit | 内容 |
|--------|------|
| `470c8f8` | Skill 推荐置顶 + 折叠按钮竞态修复 |
| `9ca2612` | 补回 `pick_project_folder` + 切换项目按钮 |
| `8f1c5f5` | 本文档初稿 |
| `1d415e7` | **Web Skill 系统实现**：bootstrap + WebSkillPanel + SkillPickerBar + ChatPanel |
| `7986b10` | WebSkillPanel loading 状态（bootstrap 完成前不空白） |
| `3dde444` | ActivityRail 显示 Skill 按钮 + JC-taijianskill-creator 增强 |
| `e26ded3` | JC-taijianskill-creator 完整对标官方 skill-creator（295行 + 3 references） |
| `f2c52f2` | WebSkillPanel CSS 全部对齐 CentralSkillsPanel |

### 涉及文件（最终）

| 文件 | 操作 | 行数变化 |
|------|------|---------|
| `public/skills/JC-taijianskill-creator/SKILL.md` | 新建 | +295 |
| `public/skills/JC-taijianskill-creator/references/schemas.md` | 新建 | +129 |
| `public/skills/JC-taijianskill-creator/references/validation-guide.md` | 新建 | +51 |
| `public/skills/JC-taijianskill-creator/references/description-optimization.md` | 新建 | +65 |
| `src/stores/agentStore.ts` | 改 | +40（bootstrap + persist + skillsBootstrapped） |
| `src/main.ts` | 改 | +7（bootstrap 调用） |
| `src/layouts/WorkspaceLayout.vue` | 改 | +3（import + 渲染） |
| `src/components/skills/WebSkillPanel.vue` | 新建 | ~300 |
| `src/components/chat/SkillPickerBar.vue` | 改 | 1 行文案 |
| `src/components/chat/ChatPanel.vue` | 改 | +6（select-skill 监听） |
| `src/components/rail/ActivityRail.vue` | 改 | 1 行（skills 移出隐藏列表） |
| `src/data/githubSkills.json` | 改 | +50（3 个自有仓库置顶） |
| `src/components/filetree/ProjectFileTree.vue` | 改 | 折叠修复 + 切换项目按钮 |
| `src-tauri/src/commands/dev.rs` | 改 | +8（pick_project_folder） |
| `src-tauri/src/lib.rs` | 改 | +1（注册命令） |

### 经验教训

1. **CSS 变量对齐是隐形坑**：WebSkillPanel 初版用了 `--jc-*` 变量（不存在），UI 全白。APP 主题用 `--surface/--ink1/--border/--paper/--olive`。未来任何 Web 端新组件必须对齐这套变量。
2. **`webHiddenTabs` 有两处**：`WorkspaceLayout.WEB_UNSUPPORTED_PANELS` 和 `ActivityRail.webHiddenTabs`，两处都要改。
3. **skill:// URI 需要 `/SKILL.md` 后缀**：直接存 markdown 全文更简单，避免二次 fetch 和路径错误。
4. **bootstrap 是异步的**：加 `skillsBootstrapped` 状态 + loading UI 避免空白闪烁。
5. **Pinia setup store 的 ref 可以直接赋值**：`store.inMemorySkills = [...]` 是合法操作，不需要 mutation。
6. **桌面端零改动是可行的**：所有新增代码在 `!isTauriRuntime()` 分支，`CentralSkillsPanel` 完全不动。
7. **纯文本 Skill vs 带脚本 Skill 是两个产品**：前者是"GPTs"，跨三端可用；后者是"Agent 工具"，桌面端专属。定位要清晰。
8. **对标官方 skill-creator 的正确方式**：不是照搬，而是逐能力评估——LLM 能模拟的改成对话模拟，不能模拟的才删除。最终保留 90%+ 内容。

---

## 七、验收标准（已通过）

1. ✅ Web 端 SkillPickerBar 下拉列表显示内置 Skill
...
7. ✅ TypeScript 编译零错误
8. ✅ UI 对齐桌面端 CentralSkillsPanel 样式
9. ✅ Rail 显示 Skill 仓库入口按钮
