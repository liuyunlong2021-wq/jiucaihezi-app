...
```json
{"schemaVersion":3,"draftPath":".raw/jc-media/文档/skill-jc-minimax-fenjing","contentHash":"...","targetSkillId":"jc-minimax-fenjing"}
```

- **出卡时**冻结该目录的内容哈希（规范化：文件路径排序 + 内容归一化）。
- **点击时**Rust 重读该路径、重算哈希、比对；不一致 → 拒绝安装并提示重新出卡。这一条保留了上一版的保护（出卡后被改过就不能装），但真相只有一份：文件。
- **兼容**：继续解析 V2 与 V1 旧卡，避免历史对话失效。
- Rust 只接受受控 token，自身校验路径：必须在项目文件树内，且目录名必须匹配 `skill-*`。
...
| 2 | 工具改读 `draft_path`，`save_skill` 出 V3 卡，Rust 接受 V3 | 一轮完整升级跑通到出安装卡，点击安装成功 |
