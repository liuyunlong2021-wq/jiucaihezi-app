...
> 现行 Raw 时序：以 [[开发/通用记忆工作台模型主导工具与审批SDD]] 为准；本轮用户消息和流式回复都先留在内存，完整回复结束后一次写入 user/assistant 对。
...
- `上传云端`：首次为当前本地项目建立云项目，并上传允许同步的文字快照。
- `同步云端`：对已绑定项目执行一次先拉后推的增量同步。

平时编辑 Wiki、Raw 和普通文字文件时，只写本地 `ProjectFileService`，不联网、不执行全量扫描、不自动上传，只更新本地“待同步路径/待同步数量”状态。打开项目、App 回到前台、网络恢复和模型工具写文件均不自动同步。

`同步云端` 执行：

1. 拉取云端 cursor 之后的变化；
2. 检查 revision 冲突并安全应用远端变化；
3. 推送本地待同步变更；
4. 更新 cursor、revision、hash 和同步状态。

媒体、凭据、Skill、MCP、Provider、Session 和设置继续不同步。

### 3.6 最小流式显示合同

Markdown/Raw 仍是对话唯一真源。流式阶段只保留一个内存中的 `streamingText`，每次收到模型返回的文字就追加并立即显示；不为流式片段创建文件、数据库记录、消息副本或第二套事件模型。

模型结束后，将本轮用户消息和完整回复作为一对一次写入当前 Raw，再用已落盘的对话内容替换临时用户/助手行。流式中断时本轮不写 Raw，原输入和附件保留供重发。
...
- 移除项目文件变化触发的自动网络同步。Wiki、Raw 和普通文字文件写入只落本地，并标记待同步路径。
- 只保留两个用户动作：`上传云端`（首次建立云项目并上传文字快照）和 `同步云端`（对已绑定项目执行一次先拉后推的增量同步）。
- 打开项目、App 回到前台、网络恢复和模型工具写文件均不自动同步；本地工作期间不发起网络请求、不扫描全量项目、不上传。
- 同一批本地变化在一次手动同步内合并处理；保留 revision、hash、mutation id、冲突副本和失败重试合同。
...
验收：同一 Wiki 填充任务在手机上完成，项目内容、云端同步和 Raw 记录与修复前一致；同一批多文件写入在一次手动“同步云端”内合并处理，工具结果不丢失；连续操作 5 分钟无明显异常升温、卡顿或无限同步。
