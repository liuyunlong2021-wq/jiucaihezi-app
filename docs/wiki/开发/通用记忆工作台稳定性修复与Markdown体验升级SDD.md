...
- 复用主 App 已验证的 `ChatScrollNav` 粘性滚动合同，不创建第二套滚动状态机。
- 流式文本变化时重新测量当前消息高度并安排跟滚；发送开始、工具轮次结束、流式结束和最终消息落盘都要经过同一滚动入口。
- 虚拟列表中的用户消息、临时流式助手消息和最终助手消息必须使用一致的高度测量和底部锚点。
- 用户主动向上滚动后停止强制跟滚；用户回到底部后恢复跟滚。

验收：连续发送长文本、触发 Wiki 工具、触发多轮工具和普通直答各 3 次；每次都能看到自己的最新输入和流式输出，结束后不跳到旧消息；主动上滚时不抢回视口。
