...
- 将项目变更同步从“每个文件事件立即完整同步”改为短窗口合并：合并同一批文件变化后只做一次扫描、拉取和上传。
- 保留现有串行队列、revision、hash 和冲突合同；只减少重复工作，不降低断网重试和数据保护。
...
验收：同一 Wiki 填充任务在手机上完成，项目内容、云端同步和 Raw 记录与修复前一致；同一批多文件写入只触发一次合并同步，工具结果不丢失；连续操作 5 分钟无明显异常升温、卡顿或无限同步。
