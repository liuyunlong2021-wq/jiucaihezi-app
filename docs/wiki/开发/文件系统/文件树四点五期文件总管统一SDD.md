...
> 前置：[[开发/文件系统/文件树一期资源身份与文件安全SDD]]、[[开发/文件系统/文件树二期批量文件操作SDD]]、[[开发/文件系统/文件树三期Explorer状态与性能SDD]]、[[开发/文件系统/文件树四期编辑区收尾SDD]]
> 后置：[[开发/文件系统/文件树五期编辑区与项目文档统一SDD]]
> 路线：[[开发/文件系统/文件树五期]]
...
## 10.2 当前记忆工作台拖放恢复与重复导入去重（2026-08-16，方案已确认、待实施）

实施测试合同：[[开发/文件系统/通用记忆工作台统一拖拽路由与附件导入去重TDD]]。

单产品分离时删除了 `WorkspaceLayout` 及其挂载的 Desktop 原生拖放分发器，但文件树和创作画布仍保留 `project:desktop-drop` 消费者，当前 `MemoryWorkbench` 输入区也只有文件选择和粘贴入口。这是 Desktop 拖放上传回归的根因，不是附件解析或 NewAPI 上传失效。

确认的最小恢复边界：

1. 当前 `App.vue` 作为唯一产品根组件，只挂载一个 Desktop `onDragDropEvent` 分发器；不恢复旧 `WorkspaceLayout`，不在多个面板重复订阅原生窗口事件。
2. 分发器继续通过 `data-project-drop-target` 路由到文件树、对话区和创作画布；对话区复用现有附件和项目资源入口，不新增上传器或附件状态。
3. Desktop Finder 路径只是一次拖放的瞬时输入，不写入 Raw、Wiki 或附件持久化元数据；成功导入后仍以项目相对 `ProjectResource` 为事实源。
4. Web 仅处理浏览器 `DataTransfer.files`；Desktop 仅处理 Tauri 原生路径。两条入口必须互斥，避免一次拖放被导入两次。Mobile 继续使用系统附件选择器，不增加桌面拖放逻辑。
5. 当前记忆工作台的持久化目录以 `.raw/jc-media/图片|视频|音频|文档` 和 `memoryMediaDirectoryFor()` 为准；10.1 中 `jc-imports/` 是分离前旧布局的实施记录，不得重新成为当前产品的第二套附件目录。

重复导入去重合同：

1. 去重发生在 `ProjectFileService` / 共享导入动作边界，文件树、对话区和画布不各自实现。
2. 只有“同一项目 + 同一素材分类 + 同一规范化文件名 + 同一 SHA-256”才认定为重复导入，直接返回已有 `ProjectResource`，不新建副本，也不发布虚假 `created` 事件。
3. 同名但内容不同必须继续使用现有重名规则保留两份；内容相同但用户明确改了文件名，也保留为独立资源，避免丢失文件名表达的语义。
4. Office/PDF 原件命中去重时，复用已有原件和已有 Markdown 可读副本；Markdown 缺失时只补生成可读副本，不再复制原件。
5. 去重不使用符号链接、硬链接、全局内容寻址仓库或绝对路径持久化；这些方案会扩大平台差异、权限和生命周期复杂度。

最小验收：同一文件连续拖入或选择两次后项目文件数不增加，两次附件都指向同一资源；同名不同内容仍保留两份；改名后的相同内容仍保留为独立资源；Desktop/Web 不重复导入，Mobile 现有选择器行为不受影响；重启、外部原件移动或删除后，已导入附件仍可从项目相对资源恢复。
