...
**根因判断**：当前 Desktop 发送要先完成连接/目录/session 处理，乐观消息和运行状态又依赖这条异步链路；因此用户点击后的第一帧没有稳定的“已收到/正在连接”投影。不是模型本身没有开始工作，而是 UI 反馈晚于发送动作。

**下一步解决方案**：

1. 发送动作进入同一帧的 `sending` 投影：立即锁定输入、显示用户消息占位和“正在连接 OpenCode”状态。
2. session 创建、权限更新、`promptAsync` 继续在后台完成；不得等待首个服务端事件才让用户看到本轮已提交。
3. 统一 `sending → connecting → busy → idle/error` 状态，不让按钮红色状态先出现而运行状态滞后。
4. 增加真实浏览器时序测试：点击发送后下一帧必须出现用户消息/发送中状态；慢 session.create 期间也不能空白。

### UX-2：本地 Ollama 比 VS Code 同模型明显更慢且 CPU 持续轰鸣

**现象**：同一个本地模型在韭菜盒子中需要较长时间并让电脑高负载；用户在 VS Code 中调用同一模型时响应更快、几乎不轰鸣。

**根因暂不能直接断言**：两边未必发送了同一请求。韭菜盒子当前可能额外携带 Skill system prompt、Wiki 上下文、工具定义、模式指令或更长历史；OpenCode 还可能先进行工具/代理规划。VS Code 可能只发送精简 prompt，或启用了不同的 Ollama 参数/上下文窗口。

**下一步解决方案**：

1. 记录两边实际请求的模型名、消息 token/字符数、system prompt 长度、工具数量、temperature、context window 和首 token 时间。
2. 先做“同 prompt、无工具、无 Skill、空历史”的基准，再逐项恢复 Skill、Wiki、工具和历史，定位额外耗时来源。
3. 文模式默认不发送本地工具定义；武模式才发送必要工具，避免本地模型为无关工具进行规划。
4. 检查 Ollama 是否被 OpenCode 代理层重复转换、重复加载模型或触发额外 compaction；以 Ollama 日志和请求时间线为准，不凭 CPU 声音猜原因。
5. 优化目标是首 token 延迟和总生成时间，不以隐藏错误或强行缩短上下文换取表面速度。
...
**根因判断**：这不是普通模型回复错误，而是 Desktop WebView 到本地 OpenCode `/global/event` 的传输层失败。可能是 sidecar 已退出/端口失效，也可能是 WebView fetch 触发 localhost CORS，而官方桌面使用了平台 fetch/Rust bridge。当前 bridge 会按固定重连节奏继续尝试，因此错误被无限刷屏。

**下一步解决方案**：

1. 先确认端口对应的 OpenCode sidecar 是否仍存活、PID 是否变化、`/global/event` 是否能用带认证的 curl 访问。
2. 对 Tauri Desktop 的 global event 明确选择可用的 Rust HTTP bridge 或官方等价 platform fetch；不能让普通 WebView fetch 承担本地 sidecar SSE。
3. 连接前做 server health/authorization 校验；sidecar 不存在时停止当前 bridge，不进入无限重连。
4. 重连采用有上限的退避和单次错误摘要，避免每 250ms 重复打印同一 CORS/connection error；新连接成功后再恢复事件流和 bootstrap。
5. 增加故障测试：sidecar 退出、端口复用、CORS 拒绝、认证失败分别只能产生一个可诊断状态，不能创建并行 global stream。

这三个问题属于下一轮体验与性能 SDD，不改变本轮已提交的消息一致性架构。当前提交保留为稳定基线，后续修复必须从 `694b5e82` 继续，不回退全局 Sync Store。
