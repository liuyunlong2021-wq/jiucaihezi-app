...
这里的“当前 Provider/K”是发送本轮主模型请求时已经解析出的同一份配置。运行时不得为了寻找 Gemini 再读取默认分组、其他分组、其他 Provider 或平台公共账号；也不得自动为用户创建、切换或拉取新的 K。用户账号里不存在可用 Gemini，不是创模式需要替用户补齐的服务缺口。
...
本地工具分支也不等于产品自动处理。产品先展示现有权限确认条；只有用户允许后，模型才能通过现有工具循环调用本地工具。当前模型不支持 function calling、用户拒绝、工具未安装或执行失败时，本轮明确结束，不能偷偷上传云端、切模型或退化成仅凭文件名猜测。
