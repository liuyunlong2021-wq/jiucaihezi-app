...
> **2026-09-24 现行覆盖说明**：本页关于 `.raw/对话记录` 为唯一真源、创建记忆空间、记忆/查询开关和 `.raw/记忆索引` 的设计，已被 [[韭菜盒子Harness会话与可选建库统一合同-2026-09-24]] 替代；其余文件树、预览、附件与平台历史仍作实施依据。

> 日期：2026-07-28
> 更新：2026-08-07（Jina 网页工具迁出）
...
> 依据：`AGENTS.md`、[[架构/产品架构]]、[[开发/通用记忆工作台单产品化分离SDD]]、[[开发/通用记忆工作台模型主导工具与审批SDD]]、[[开发/通用记忆工作台原始素材与文档按需阅读SDD]]、[[开发/Wiki四Skill产品化升级SDD]]、[[开发/文件系统/索引]]、[[开发/文件系统/文件树一期资源身份与文件安全SDD]]、[[开发/文件系统/Web云端项目Wiki媒体同步与APP升级SDD]]、[[开发/韭菜盒子原生媒体编排能力SDD]]
...
项目骨架、四类素材归档、隐藏目录和目录保护以 [[开发/通用记忆工作台原始素材与文档按需阅读SDD]] 为唯一事实源。主 Studio、旧主 Web、媒体工作台或本页历史实施记录若与其冲突，一律不得覆盖该合同。
...
- 选择模型和一个可选专业 Skill。
- 在记忆模式上传当前模型支持的附件。
- 每轮可选择快速或记忆；两种模式共享当前对话上下文，记忆模式额外提供 Raw、Wiki、Skill 和项目工具。
- 明确同意后，把确认内容写入指定或确认的 Wiki 路径。
- 在记忆模式用自然语言生成图片、视频或音频，并在对话内确认费用、查看进度和结果。
...
| 原始文档与媒体 | 当前项目 `.raw/jc-media/{文档,图片,视频,音频}/` |
| 在途媒体任务 | 现有 `mediaTaskStore` |

App 不再把同一份对话正文保存进 conversations/messages 数据库。清除页面状态后，只要项目目录还在，就必须能从 Raw 恢复全部对话，从 Wiki 恢复确认知识，从 `.raw/jc-media/` 恢复原始文档与媒体。第四阶段的 D1 是跨设备传输和恢复副本，不是模型直接读取的新记忆库；同步下载后仍由 `ProjectFileService` 读写本地文件。
...
- 已确认媒体计划；用户确认生成的动作与最终成功、失败或取消回执作为完整问答对；
- 用户确认了哪项 Wiki 写入。
...
1. 文档、图片、视频和音频分别写入 `.raw/jc-media/` 对应中文分类目录，并将原文件只写入一次；不再按上传或生成来源拆目录。
2. Raw 附件元数据保存 `id / name / mime / size / kind / projectPath`；`projectPath` 必须位于 `.raw/jc-media/` 对应分类，且不含 `.`、`..` 或空路径段。
3. Raw 不保存 Base64、Blob URL 或远程鉴权 URL；项目文件是附件二进制唯一真源。
4. 当前轮附件默认显式选中；历史附件不在每轮自动注入，模型需要时先按需读取 Raw 中的 `projectPath`，再只物化本次选中的 `referenceId`。
...
这是一套 Wiki 项目文件系统内的稳定引用关系，不是第二套附件数据库：Raw 记录附件在哪轮出现，`.raw/jc-media/` 对应分类保存附件本体，媒体计划保存受控引用。
...
```text
用户点击发送
  -> App 从当前选中的对话 Raw 解析已完成轮次
  -> 按当前模型的上下文容量，从最新完整问答轮次向前装入；装得下时装入当前对话全部历史，再追加本轮用户消息，不写本轮 Raw
  -> 快速：发送该上下文，只提供只读 wiki_search；不提供附件、项目引用、网址读取或其他工具
  -> 记忆：发送同一上下文和本轮附件，并提供当前 Raw 路径与 Wiki/Skill/项目/MCP 等候选工具
  -> 记忆模式下，模型按 Skill description 自主判断是否调用 skill(name) 加载 SKILL.md
  -> 记忆模式下，模型按任务需要决定是否读取 Raw、查询 Wiki 或调用其他工具
  -> 模型生成最终回答
  -> App 以一次 revision 把用户消息 + 完整最终回答追加到当前 Raw
```

“读取当前对话 Markdown”和“按容量装入本轮上下文”不是两套历史：前者确定唯一持久化来源，后者从这同一份 Raw 的解析结果中选择本轮能容纳的完整问答轮次。App 不做语义检索，不预查 Wiki，不把固定查询结果拼进请求；最新用户消息只出现一次。失败、取消或流式中断时 Raw 不写半轮，输入和附件仍可修改或重发。

`快速` 和 `记忆` 共用同一份 Raw 和当前对话上下文；开关只影响下一次发送可用的能力。新对话默认 `记忆`，重开对话恢复最后一个带模式的用户 turn。

新建或切换对话后，直接上下文只从当前选中的 Raw 解析，不自动装入前一个或其他对话。快速模式只有 `wiki_search`，不能读取其他 Raw；记忆模式初始上下文同样只含当前 Raw，但其项目级 `read/grep` 候选工具在权限上可以按任务主动读取其他 Raw，因此这里是默认隔离，不是权限级绝对隔离。
...
输入框的 `@` 是统一的本轮引用入口，动态列出当前项目可用文件和用户已安装 Skill。选择项目文档后读取真实内容作为本轮 `files` 引用；选择项目媒体后复用现有附件物化合同；选择 Skill 后自动进入记忆模式，并在首轮模型请求之前用标准 assistant tool call + tool result 协议精确执行 `skill(name)`。这是用户明确选择，不是关键词路由，也不把产品内置 Skill 暴露到选择器。切换到快速模式必须清除已选 Skill、附件和项目引用。Jina 网页读取与搜索已于 2026-08-07 从三端迁出；Desktop 联网按需使用需审批的本机 Terminal，Web 与 Mobile 不提供替代网页工具。

模式选择器是工具权限的唯一来源，用户消息中的文字不得开启、关闭或裁剪工具池。快速模式基于当前上下文、模型自身已有的知识和按需只读 Wiki 查询回答；只允许调用 `wiki_search`，不得调用其他工具或访问 Wiki 之外的项目资料。记忆模式始终提供完整候选工具，即使用户文字写“不要调用工具”也不会移除工具；模型仍按当前任务自主决定实际是否调用。两种模式都需要当前模型支持工具调用。

### 7.3 上下文

- Raw 永远保存已经完成的完整问答轮次，不保存失败发送尝试。
- 打开或切换对话时，App 只读取当前选中的 Raw 并解析为 `ConversationTurn[]`；不存在第二份历史数据库。
- 发送时按当前模型上下文容量，从最新完整问答轮次向前装入；能装下时把当前对话全部历史装入。这是普通模型消息，不是工具调用。
- 记忆模式在同一上下文上额外提供当前 Raw 路径和完整候选工具；快速模式只提供只读 `wiki_search`，不提供附件或其他项目资料。
- 不创建隐藏摘要、第二套记忆或专用“对话过长”流程；上下文只使用当前 Raw 的真实轮次，并受当前模型上下文窗口约束。
- 不自动拼入其他对话；快速模式不能读取其他 Raw，记忆模式只有在模型实际调用项目级 `read/grep` 时才可能读取。
- Provider 返回错误时如实显示，不伪造回答，不写本轮 Raw。
...
快速模式不接收附件、不生成媒体；上传、引用或生成媒体时必须进入记忆模式。

媒体生成是基础能力，不依赖创作面板或特定 Skill。用户要求生成图片、视频或音频时：

```text
模型按当前快速/记忆合同回答
  -> 上传附件先写入当前项目 .raw/jc-media/ 对应分类
  -> 当前 Raw 记录项目相对路径
  -> 本轮及本次按需选中的历史附件注册为受控 referenceId
  ...
  -> 结果写入当前项目 .raw/jc-media/ 对应分类
  -> 用户确认生成的动作 + 最终回执作为完整问答对写入 Raw
```
...
- 成功回执以项目文件为真源，显示 `已保存到 .raw/jc-media/...`，并只提供预览、下载副本和在文件树中查看；不再显示含义重复的“保存”。
- Raw 只写成功任务的项目相对路径，不写 CDN URL；远程 URL 只是内部下载源和旧任务降级通道。
- 媒体计划的生成按钮、任务图标、进度条和操作悬停色统一使用 `--olive*` 主题变量，不写死紫色。
- 任务状态库丢失时，已成功媒体仍可从 Raw 相对路径和 `.raw/jc-media/` 恢复。
...
模型返回 MediaPlan
  ...
  -> mediaTaskStore 冻结任务快照、轮询并落当前项目 .raw/jc-media/ 对应分类
  -> 进度与历史只在 CreationPanel 显示
  -> 用户确认生成的动作与最终成功/失败回执作为完整问答对写入 Raw
```
...
| `MemoryChatRuntime` | 当前 userTurn 请求装配、Raw 路径提供、Wiki/Skill/MCP 工具循环和完整问答轮次落盘 |
...
| `.raw/jc-media/**` 二进制原件、图片、视频和音频 | 否 | 只留在生成或上传它的设备；允许范围内的 Markdown 可读副本按专项 SDD 同步 |
| 用户 Skill、MCP、Provider、API Key、Session、界面设置 | 否 | 继续使用各平台现有安全存储；凭据绝不进入同步库 |
| 二进制、未知格式、`.env` 和其他敏感文件 | 否 | 不读取、不上传、不猜测 |

另一设备打开含媒体引用的 Raw 时，正常显示文字和一张“媒体仅保存在原设备”的占位卡，不把缺失的 `.raw/jc-media/...` 当作损坏项目，不自动访问历史临时 URL。用户可在当前设备重新上传或重新生成媒体。
...
- 两个方向性动作都以用户本次明确选择的一侧为准；不合并正文、不生成冲突副本，也不自动判断哪一侧更新。
- 服务端内部仍使用 `expectedRevision` 防止请求执行期间被并发写穿；收到 `409` 时重新读取云端快照并按本次方向重试，最后一次用户动作获胜。
- 删除使用 tombstone，至少保留 30 天，防止离线设备把旧文件重新上传；恢复和永久删除后续复用同一 revision 合同。
- 覆盖删除只作用于 `isSyncableTextPath` 允许的文字文件；媒体、空目录、凭据、设置、Skill、MCP、Provider、Session 和 `.raw/.sync` 不比较、不上传、不下载、不删除。
...
1. `上传并覆盖云端`：未绑定时先为当前本地项目创建稳定 `cloud project_id`；随后以本地允许同步的完整文字快照覆盖该云项目。云端同路径文字被覆盖，云端独有文字被删除。
2. `下载并覆盖本地`：以云端允许同步的完整文字快照覆盖当前本地项目。用户首次从云项目列表进入时，Web 创建浏览器本地副本，Desktop 选择保存位置，Mobile 写入 App 管理目录；已绑定项目可重复执行。云端同路径文字覆盖本地，本地独有可同步文字被删除。

两个动作执行前必须明确提示覆盖方向和删除后果；按钮标题本身不得缩写为“上传云端”“下载云端”“同步云端”或“立即同步”。打开项目、App 回到前台、网络恢复和本地保存均不自动传输。

项目入口显示云端绑定状态和原生进度条：`正在扫描 N/M / 上传 N/M / 下载 N/M / 上传并覆盖云端完成 / 下载并覆盖本地完成 / 操作失败`，并固定说明“只处理文字；媒体和空目录不处理”。设置页只显示账号、当前绑定、最近结果、进度和范围，不提供任何上传、下载或同步按钮。
...
| 情况 | 行为 |
| --- | --- |
| 模型失败、取消或流式中断 | Raw 不写本轮，保留原输入和附件供修改或重发 |
| 完整问答轮次 Raw 追加失败 | 保留输入和可见结果并阻止下一轮，以相同 turn ID 重试 |
| Raw / Wiki / MCP 工具读取失败 | 把真实错误返回模型；不伪造资料，也不把“未调用工具”当回复门禁 |
| Wiki 审批拒绝或写入失败 | 不声称写入成功；模型若形成完整最终回复，仍按完整问答轮次落 Raw |
| 媒体失败或取消 | 用户确认动作与真实最终回执作为完整问答对落 Raw，不写成功路径 |
| 媒体成功但项目落盘失败 | 保留生成成功与原始结果，单独显示保存警告和“重试保存” |
| 多窗口同时追加 | revision 冲突后重读，按 turn ID 去重重试 |
| 文字传输离线或超时 | 本地写入照常成功；本次方向性操作失败并明确提示，用户可重新点击原动作 |
| 云端 revision 冲突 | 重新读取云端快照并按用户本次选择的覆盖方向重试；不合并、不生成冲突副本 |
...
3. 客户端复用 `textSyncClient + ProjectFileService`，按用户选择读取本地或云端完整文字快照，不接管底层文件系统。
4. 把云项目并入应用级项目入口；项目中心只提供“上传并覆盖云端”和“下载并覆盖本地”，设置页只显示状态。
5. 先用同一账号完成 Web 上传覆盖 -> Mac 下载覆盖、Mac 上传覆盖 -> Web 下载覆盖、离线失败重试和排除路径真实闭环。

验证：新设备无需预先创建同名项目或理解本地 owner；从云项目列表下载后即可顺序继续同一 Raw 对话和 Wiki。源侧已删除文字在目标侧不会复活，断网不丢本地内容，不同步或删除任何媒体、空目录或凭据，不同账号无法探测项目是否存在。
...
| 4C-9 | 云项目列表、下载、上传和文字同步 | 同账号云项目可下载为 App 管理目录副本、上传本地项目并先拉后推 Raw/Wiki/安全文本；重启恢复映射。 | 历史已完成；2.1.17 下载回归待修复 |
...
| 4C-13 | TestFlight 构建与真机验收 | 4C-7 至 4C-12 全部通过后归档上传 TestFlight；从 TestFlight 安装并完成同一真机回归。 | 已完成 |

4C-9 与下方 2026-07-26 至 2026-07-28 的同步实施记录是当时增量双向合同的真实历史证据；该产品合同已由 2026-08-04 的方向性覆盖合同替代，不能再据此恢复“同步云端”“立即同步”、先拉后推或冲突副本。

2026-08-10 真实 iPhone 回归发现：`2.1.17` 开发签名版点击云端项目并确认“下载并覆盖本地”后没有可见结果，四个本地项目的 `.raw/.sync/state.json` 修改时间均未变化。多轮局部修改、自动测试、IPA 构建、安装与启动均未形成真实下载落盘，因此 4C-9 当前重新打开，详见 [[排障/iPhone云项目下载覆盖本地无响应-2026-08-10]]。
...
- 对话创建、H1 标题、Raw 追加、修订冲突重试和文件树切换已落地；当时实现为用户消息先落 Raw、回复完成后再落 assistant，该历史时序已由第 7 节现行目标替代。
...
- 输入框增加逐轮 `快速 | 记忆`；两者共用同一 Raw，快速不加载 Wiki/MCP 工具，记忆当时采用强制查询，旧 Raw 默认记忆。该条是历史实现记录，现行目标已由第 7 节的模型按需读取合同替代。
...
- 两条路径共享当前 Raw 解析出的对话上下文；快速路径额外只有只读 `wiki_search`，记忆路径提供当前 Raw、Skill、项目和 MCP 等候选工具。
- “快”来自只保留 Wiki 查询、不提供附件、项目引用、网址读取和其他工具，不是来自丢弃当前对话上下文。
...
- 页面内存中的 Data URL 只服务当前轮预览和模型识图，不是长期记忆。刷新恢复、隔轮复用和卡内换模型必须依赖 `.raw/jc-media/ + Raw projectPath`。
...
媒体任务：状态、模型、提示词、参数、进度、错误
  ...
  -> 轻量载体：对话卡与画布按需消费元数据和缩略图；文件树只消费元数据和类型图标
...
- Desktop、Web、Mobile 共享上述身份和生命周期合同；项目文件树一律只显示类型图标，不读取、不生成、不缓存缩略图。对话卡与画布的按需预览按各平台能力实现。
...
### 14.20 V2.1.0 发布前跨端收口记录（2026-07-28）

- Windows 启动误报的根因是前端按浏览器 UA 猜测 WebView2：只要不是 Edge 就弹出“可能缺少 WebView2”，导致 Web 与已正常启动的 Tauri WebView 都可能被误判。该猜测已删除；真正缺少 WebView2 时前端 JS 无法执行，由 Tauri/Windows 原生启动链负责提示。
- Mac 创作面板提示词只在键盘输入事件中调整高度，程序恢复的长提示词不会触发。现改为同时监听提示词状态与输入框显示状态，在 DOM 更新后复用同一 `scrollHeight` 逻辑，最大高度仍为 200px；iPhone 原有输入体验不变。
- 画布导航按用户确认的产品规则收口：创作面板复用“当前记录选择器 + 新建”的成熟对话 UI，提供画布搜索、切换、重命名、删除和新建；所有画布仍只保存为项目 `jc-canvas/*.jccanvas`，没有第二份历史数据库。文件树点击继续使用统一临时预览，不因 `.jccanvas` 特例自动切换创作面板；文件树显式“加入画布”动作仍保留。
- 画布重命名和删除继续经过现有 `canvas:before-rename` / `canvas:before-delete` 保存屏障；当前唯一画布删除后仍由既有逻辑创建替代画布。未增加依赖或移动端专属存储。
- 自动验证：相关组件、文件树与记忆工作台合同 100 项中 99 通过、1 项既有跳过；TypeScript、Desktop quick build、Desktop 产物审计和 `git diff --check` 通过。Mac debug App/DMG 构建和签名成功，没有覆盖 `/Applications` 正式版。
- iPhone 开发 IPA 构建、导出、覆盖安装和启动成功；SHA-256 为 `7d70a535bff65b6c329af23c41779dbeaef3430e4546151332fda3f4bb00496b`，大小 42,547,724 bytes，安装后 PID 2403 存活。用户在真实 iPhone 13 Pro Max 确认画布记录、新建、切换和文件树统一预览全部通过。
...
- 已创建自动分发的内部群组“4C-13 内部验收”，构建 `2.1.0.1` 已加入，并邀请账户持有人测试。发布侧完成归档、正式签名、上传和苹果处理。
...
5. 图片、视频、音频分别可预览或播放。
6. 二进制、超大文本和 `.jccanvas` 不进入聊天或可写文档；文件树点击统一进入安全临时预览，画布编辑与历史切换只从创作面板进入。
...
11. 创作面板以当前画布选择器和“新建画布”管理项目 `jc-canvas/*.jccanvas`，支持搜索、切换、重命名和删除，不建立第二份画布记录存储。

### 15.2 对话与 Wiki

1. 新建对话立即创建带有效标记的唯一 Raw。
2. 打开或切换对话只读取当前选中的 Raw；发送时按模型容量从最新完整问答轮次向前装入，能装下时装入当前对话全部历史，再追加本轮用户消息。完整回复成功后，本轮用户消息与助手回复才以一个完整轮次写入 Raw。
3. 快速模式的请求只包含只读 `wiki_search`，不包含附件、项目引用、网址读取或其他工具；记忆模式在同一上下文上提供当前 Raw 路径和完整候选工具。
4. 模式选择器是工具权限的唯一来源，用户消息中的文字不能改变工具池；模型只负责在已提供的候选工具中决定是否实际调用。
5. 记忆模式下，模型看到当前任务和 Skill 目录后，按 description 自主决定是否调用 `jc-cha-wiki` 或其他 Skill；App 不预查关键词、不强制调用。
6. 普通聊天不写 Wiki；用户确认后可写合理路径并追溯到 Raw turn。
7. 清除 UI 状态后，全部对话仍可从 Raw 恢复。
8. 记忆模式可把普通文本文件引用到当前轮请求，原文件保持不变；快速模式不接收项目文件引用。
9. 绑定普通文件夹不自动写入；点击“新建记忆空间”后才建立 Wiki/Raw，点击“新建对话”后才建立对话 Raw。
10. 模型失败、取消或流式中断时 Raw 不写半轮，原输入和附件保留供修改或重发；不增加完成检测器或自动重试模型。
11. 新对话不自动装入前一个或其他对话；快速模式不能读取其他 Raw，记忆模式只有主动调用项目级 `read/grep` 才可能读取。
...
5. 结果分别写入 `.raw/jc-media/图片`、`视频`、`音频`，Raw 只记项目相对路径。
...
11. 上传或粘贴的图片、视频立即保存到 `.raw/jc-media/` 对应分类，Raw 只保存项目相对路径和附件元数据，不保存二进制副本。
12. 隔多轮后可用明确指代复用历史上传素材；只有一个候选时直接进入带缩略图的确认卡，多个歧义候选时不得静默选错。
13. 刷新页面、重新打开对话或在原确认卡切换模型和参数后，图生图/图生视频仍使用原项目素材。
14. 上游生成成功但项目保存失败时，结果仍按成功展示，并提供原始结果与“重试保存”；不得显示为“生成失败”。
15. 韭菜盒子 API 的受保护结果下载携带当前 Token，第三方媒体域名绝不携带该 Token；播放优先使用项目内已保存副本。
16. 对话、创作面板、画布和文件树对同一素材只保存项目资源引用；创作面板直接导入或 `@` 选择媒体后，该素材写项目一次并在画布中选中。
17. 生成成功不自动向画布追加节点；用户明确“放到画布”后按结果真实宽高比显示，不得沿用参考图边界拉伸结果。
...
8. 当前发布顺序为 V2.1.0 Web/桌面正式版、4C-13 TestFlight、Android；前一阶段必须有真实产物和安装/访问证据，不能用 Git push 或本地构建代替上线。

### 15.6 跨设备文字同步与 Mobile

1. 同一账号在 Web、Mac 和 Mobile 看到相同的云项目列表；不同账号即使猜中 project ID 也只能得到统一的不可见响应。
2. 本地项目可通过“上传并覆盖云端”建立云项目；另一设备从云项目进入并创建新的本地副本、保存映射并直接打开，不要求用户先创建同名项目。
3. 项目中心只有“上传并覆盖云端”和“下载并覆盖本地”两个方向性数据动作；设置页没有操作按钮，打开项目和联网不自动传输。
4. `.raw/对话记录/`、Wiki 和允许的普通文本按用户选择的方向做全量快照覆盖；媒体二进制、空目录、Skill、MCP、Provider、API Key、Session、设置和 `.raw/.sync` 不会进入请求或删除范围。
5. 手机新增一个用户 turn 和完整 assistant turn 后，Mac 拉取即可继续同一对话；Mac 写入 Wiki 后，手机拉取能用于下一轮记忆查询。
6. 断网或在线发送和 Wiki 写入都先在本地成功；只有用户点击“上传并覆盖云端”才上传，点击“下载并覆盖本地”才下载。
7. 两台设备修改同一文件时不自动合并；用户最后明确执行的方向性覆盖动作决定该次结果，不生成冲突副本。
8. 删除本地副本不删除云项目；云端删除同步到离线设备后不会被旧副本复活，30 天 tombstone 期间可以恢复。
9. 新设备打开含 `.raw/jc-media/...` 引用的对话时，文字和计划正常显示，缺失媒体明确标注“仅保存在原设备”，无自动远程下载和崩溃。
...
13. 上传覆盖 205 个不同路径时必须形成 `100 / 100 / 5` 三批请求；界面依次显示扫描、上传或下载的完成数/总数，结束后显示明确方向和变更数量，并说明媒体与空目录不处理。
...
打开 https://jiucaihezi.studio
  ...
  -> 点击顶部“新建对话”并开始聊天
  -> 用户消息作为本轮唯一当前任务交给模型
  -> 模型按需读取当前 Raw 并自主查询 Wiki
  -> 完整回复成功后，用户消息与助手回复作为一轮写回同一个 Raw
  ...
  -> 在对话内确认并生成媒体，结果落当前项目
  -> 用户确认后把结论写入 Wiki
  -> 点击“上传并覆盖云端”，建立 cloud project_id 并以本地文字快照覆盖云端
  -> Mac/Mobile 登录同一账号，在项目入口看到该云项目
  -> 从云项目进入，创建本地副本并直接打开同一 Raw/Wiki
  -> 手机编辑 Raw/Wiki 后，点击“上传并覆盖云端”
  -> Mac 点击“下载并覆盖本地”，以云端文字快照覆盖本地文字
...
## 18. Web / Mobile 核心能力收敛与三端创作面板资源边界（2026-08-03）

> 状态：已实施并通过自动验证。本节只裁剪 Web / Mobile 运行时能力；Desktop 仅统一创作面板关闭时的资源释放，不裁剪任何工具能力。

### 18.1 决断与目标

Web / Mobile 不是普通聊天壳，也不复制 Desktop 全部能力。两端保留产品核心的“项目 + 对话 + Wiki + 附件 + 云媒体”，去掉依赖本机终端、桌面运行时或持续占用图形资源的能力。继续复用现有记忆工作台、文件服务、Wiki Skill、媒体任务和文字同步合同，不建立第三套模式、第二套媒体引擎或平行数据结构。三端创作面板统一采用“打开时挂载、关闭前保存、关闭后卸载”，避免隐藏面板继续占用 LeaferJS、Canvas 和媒体资源。

### 18.2 三平台能力矩阵

| 能力 | Desktop | Web / Mobile |
| --- | --- | --- |
| 当前项目、当前对话 Raw 与上下文 | 保留 | 保留 |
| Wiki 查询、读取、写入、修正、巡检、一致性检查、关系图 | 保留 | 保留 |
| 项目内 `read / glob / grep / write / edit` | 保留 | 保留，必须受当前项目根目录约束 |
| 附件上传、图片、Office / PDF 转 Markdown、站内预览 | 保留 | 保留 |
| 标准 `.canvas` Wiki 项目地图 | 保留 | 保留，按需打开 |
| 云端图片、视频、音频生成 | 保留 | 保留 |
| Markdown、HTML、PDF、PPTX 等按需导出 | 保留 | 保留平台实际可用的既有出口 |
| `.jccanvas` 云媒体创作面板 | 保留；关闭前保存并卸载 | 可保留；关闭前保存并卸载 |
| `.jcscene`、Three.js 编辑器与播放器 | 保留 | 不提供 |
| 本机 FFmpeg MP4 导出 | 保留 | 不提供 |
| 本地终端、本地模型、本地 `stdio MCP` | 保留 | 不提供 |
| 自定义 MCP / Skill 仓库管理 UI | 保留 | 可隐藏；不得影响内置 Wiki Skill |

文字同步继续覆盖 Raw、Wiki 和允许的项目文本；图片、音频、视频及其他媒体二进制绝不同步。

### 18.3 Web / Mobile 工具白名单

Web / Mobile 记忆模式候选工具只来自以下既有能力：

1. Wiki 工具：查询、读取、写入、修正、巡检、一致性检查和关系图。
2. 当前项目根目录内的 `read / glob / grep / write / edit`。
3. 现有附件读取、文档转换和站内预览链路。
4. 现有云端图片、视频、音频生成及已支持的文档导出。

不得向 Web / Mobile 暴露 `create_3d_scene`、`.jcscene` 运行时、本机 FFmpeg、Terminal、本地模型或本地 `stdio MCP`。快速模式仍只提供既有只读 `wiki_search`，本节不改变快速/记忆模式合同。

### 18.4 UI 与按需加载边界

- 三端只有用户主动打开时才加载创作面板；Web / Mobile 的项目地图和媒体预览也继续按需加载。
- 三端关闭创作面板时必须先完成当前 `.jccanvas` 保存，再卸载 `CreationPanel`，释放 LeaferJS 实例、事件、Canvas、运行时媒体 URL 和画布写入锁；不能直接用 `v-if` 丢弃尚未完成的延迟保存。
- 媒体生成和轮询继续由现有 `mediaTaskStore` 管理，创作面板关闭不得取消任务；重新打开后从任务状态和已保存 `.jccanvas` 恢复。
- 普通对话不得常驻 2D 或 3D 编辑器资源。
- 隐藏不可用入口，不保留点击后才提示“不支持”的假能力。
- 不改变现有对话显示、Raw 持久化和屏外绘制优化。

### 18.5 三类空间文件分工

| 文件 | 唯一职责 | 跨端边界 |
| --- | --- | --- |
| `.canvas` | Wiki 项目地图：节点、连线、关系和导航 | 三端按需查看与维护 |
| `.jccanvas` | LeaferJS 2D 媒体生产画布：图片、箭头、文字、画笔、图层 | 三端按需挂载；关闭前保存，关闭后卸载 |
| `.jcscene` | Three.js 3D 场景、机位和动画 | 仅 Desktop |

三者不互相替代，不把 Wiki 地图塞进创作面板，也不把 2D 标注工具迁入 Three.js。

### 18.6 最小实施步骤

1. 在 Web / Mobile 工具装配入口使用白名单，移除 `create_3d_scene` 及所有本地运行时工具；Desktop 工具装配不变。
2. 按平台隐藏 3D、本地终端、本地模型、本地 MCP 和相关不可用入口；保留内置 Wiki Skill。
3. 给现有创作面板关闭动作增加保存屏障：保存成功后，三端统一卸载；媒体任务仍由 `mediaTaskStore` 继续运行。
4. 用现有三端合同测试覆盖工具清单、项目根目录限制、关闭保存、资源卸载、任务继续运行和重新打开恢复。

### 18.7 验收标准

1. Desktop 的工具、Three.js 和 FFmpeg 能力与实施前一致；创作面板关闭时先保存并释放资源，重新打开能恢复同一 `.jccanvas`。
2. Web / Mobile 能在当前项目内完成 Wiki 查询、读取、写入、修正、巡检、一致性检查和关系图操作。
3. Web / Mobile 能上传附件、转换支持的 Office / PDF、站内预览，并继续使用云端媒体生成。
4. Web / Mobile 请求中不存在 `create_3d_scene`、FFmpeg、Terminal、本地模型和本地 `stdio MCP` 工具。
5. 三端关闭创作面板后，LeaferJS / Canvas 不再挂载；尚未完成的画布保存不能丢失，普通长对话不加载 2D 或 3D 编辑器。
6. 创作面板关闭后，已提交媒体任务继续运行；重新打开能看到真实任务状态和已落盘结果。
7. 同步请求只包含允许的文字文件，不包含任何媒体二进制、Base64 或本地绝对路径。
8. Desktop、Web 浏览器与真实 iPhone 分别完成创作面板关闭/恢复；Web 与 iPhone 另完成长对话、Wiki 读写、附件和云媒体人工验收，无崩溃或持续异常发热。

### 18.8 明确不做

- 不裁剪 Desktop 的工具和本地能力；只修改创作面板关闭时的保存与资源生命周期。
- 不删除或重构现有创作面板。
- 不新增依赖、平台模式、同步协议、媒体后端或第二套项目文件服务。
- 不把媒体二进制加入云同步。
- 不把自动验证替代 Desktop、Web 浏览器和真实 iPhone 的人工矩阵。

### 18.9 实施记录

2026-08-03 已按既有运行时边界完成最小实施：Web / Mobile 记忆工具清单移除 `create_3d_scene` 和自定义 MCP，Desktop 工具装配不变；Web / Mobile 文件树与旧对话隐藏 `.jcscene` 入口，自定义 Skill / MCP 管理只在 Desktop 设置中显示，内置 Wiki Skill 继续可用。
...
### 20.5 外部附件路径与重复导入决策（2026-08-16，已确认、待实施）

1. Desktop 文件选择器或 Finder 拖放提供的绝对路径只在当次导入过程中使用；不写入 Raw、Wiki、对话附件元数据或可同步文本。
2. 附件首次成功导入后，持久化身份仍是当前项目内的相对 `ProjectResource`；外部原文件之后移动、改名、删除或卸载磁盘，不影响已保存对话的附件恢复。
3. 同一附件重复选择、拖入或再次发送时，共享导入边界按“同项目、同分类、同规范化文件名、同 SHA-256”复用已有资源，不再生成无限重名副本。
4. 同名不同内容必须保留新副本；内容相同但用户明确改名也保留为独立资源。不以文件名、文件大小或修改时间单独判断内容相同。
5. Office/PDF 仍保留“项目原件 + Markdown 可读副本”；原件去重命中时复用已有可读副本，缺失时只补生成 Markdown，不再复制原件。
6. 本决策不新增绝对路径引用系统、全局素材仓、符号链接、硬链接或第二套附件存储。详细导入和验收合同见 [[开发/文件系统/文件树四点五期文件总管统一SDD#10.2 当前记忆工作台拖放恢复与重复导入去重（2026-08-16，方案已确认、待实施）]]。
