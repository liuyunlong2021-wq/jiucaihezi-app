...
本地创建/修改/删除安全文本
  ...
  -> 标记本地待同步路径（mutation id + expected revision）
  -> 不发起网络请求

用户点击“同步云端”
  -> 按 cursor 拉取远端变更
  -> D1 原子校验账号、路径、mutation id 和 revision
  -> 安全写入本地 ProjectFileService
  -> 按路径顺序、每批最多 100 项推送本地待同步变更
  -> 返回新 revision / 冲突并更新 cursor
```
...
1. `上传云端`：为当前本地项目创建稳定 `cloud project_id`，上传允许同步的完整文字快照，并保存本地映射。
2. 从云端项目进入并创建本地副本：用户从云项目列表选择项目；Web 创建浏览器本地副本，Desktop 选择保存位置并创建同名项目目录，Mobile 写入 App 管理目录；完整拉取成功后保存本地映射并打开项目。该过程是首次接续的内部流程，不作为日常第三个同步按钮。

完成首次上传或创建本地副本后，后续只有 `同步云端`：先按 cursor 增量拉取，再按本地待同步队列增量推送。不得把“再次下载”实现为删除本地目录或用云端快照静默覆盖；同路径双方均有修改时继续使用 revision 冲突与可见冲突副本合同。

本地文本成功写入后只进入待同步状态；`同步云端`是用户手动执行或失败重试的入口，不是日常保存按钮。打开项目、App 回到前台或网络恢复时不自动同步。断网或在线都先本地保存，待用户点击 `同步云端` 后沿用原 mutation id 上传。
...
| 文字同步离线或超时 | 本地写入照常成功，保留待同步状态；用户下次点击“同步云端”时按同一 mutation id 重试 |
...
4. 把云项目并入应用级项目入口，完成“本地项目上传云端”和“云项目创建本地副本”；设置页不再负责手工配对两个 owner，日常只提供“同步云端”。
...
2. 本地项目可通过“上传云端”建立云项目；另一设备从云项目进入并创建新的本地副本、保存映射并直接打开，不要求用户先创建同名项目。
3. 首次接续后只有用户点击“同步云端”才做增量拉取/推送；重复同步不得删除整个本地项目或用云端快照静默覆盖未上传变化。
...
6. 断网或在线发送和 Wiki 写入都先在本地成功并显示“待同步”；只有用户点击“同步云端”后才按同一 mutation id 上传，不生成重复 turn 或重复文件。
...
打开 https://jiucaihezi.studio
  ...
  -> 在对话内确认并生成媒体，结果落当前项目
  -> 用户确认后把结论写入 Wiki
  -> 点击“上传云端”，建立 cloud project_id 并上传允许的文字文件
  -> Mac/Mobile 登录同一账号，在项目入口看到该云项目
  -> 从云项目进入，创建本地副本并直接打开同一 Raw/Wiki
  -> 后续本地 Raw/Wiki 变更只标记待同步
  -> 用户点击“同步云端”，先拉后推
