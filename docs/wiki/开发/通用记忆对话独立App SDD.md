...
> 日期：2026-07-28
> 更新：2026-07-31（当前任务、按需 Raw、完整问答轮次落盘合同）
> 状态：V2.1.3 Web 与桌面正式版已发布；当前任务 / Raw 历史隔离和 `.raw/jc-media` 目录迁移待实施，Android 继续暂停。
...
- 选择模型和一个可选专业 Skill。
- 上传当前模型支持的附件。
- 每轮可选择快速处理当前独立任务，或在记忆模式中由模型按需读取当前 Raw、查询 Wiki 后回答。
...
```text
用户点击发送
  -> App 在本轮 UI 中保留用户消息、附件及 quick | memory 模式，不写 Raw
  -> 快速：只发送本轮消息/附件，无 Raw、Skill、Wiki 或 MCP 工具
  -> 记忆：发送本轮消息/附件，并提供当前 Raw 路径与 Wiki/Skill/MCP 工具
  -> 模型按 Skill description 自主判断是否调用 skill(name) 加载 SKILL.md
  -> 模型按任务需要决定是否读取 Raw、查询 Wiki 或调用其他工具
  -> 模型生成最终回答
  -> App 以一次 revision 把用户消息 + 完整最终回答追加到当前 Raw
```

App 不在模型之前猜关键词，不提前查询 Raw/Wiki，不把固定查询结果拼进请求。最新用户消息只出现一次。失败、取消或流式中断时 Raw 不写半轮，输入和附件仍可修改或重发。

`快速` 和 `记忆` 的成功问答共用同一份 Raw；开关只影响下一次发送。新对话默认 `记忆`，重开对话恢复最后一个带模式的用户 turn。快速适合不依赖前文的独立问题；用户要求继续或修改已有内容时使用记忆模式，由模型按需读取 Raw。
...
- 记忆模式只提供当前 Raw 路径，模型按需使用 `read/grep`；快速模式不读取历史。
- 不创建隐藏摘要、第二套记忆、历史容量装配或专用“对话过长”流程。
...
- 快速路径只保留本轮任务、附件和媒体规划，不提供历史或工具；记忆路径提供当前 Raw、Skill、项目和 MCP 工具，由模型按任务自主决定是否调用。
- “快”来自不读取历史和不进入工具循环；附件、媒体、模型选择以及已经完成的 Raw 历史仍被正常保存，不因快速模式删除。
...
2. 用户消息作为本轮唯一当前任务交给模型；完整回复成功后，用户消息与助手回复才以一个完整轮次写入 Raw。
3. 模型需要前文时通过现有 `read/grep` 按需读取当前 Raw；Raw 历史不再展开为仍生效的聊天指令。
4. 模型看到当前任务和 Skill 目录后，按 description 自主决定是否调用 `jc-cha-wiki` 或其他 Skill；App 不预查关键词、不强制调用。
...
9. 模型失败、取消或流式中断时 Raw 不写半轮，原输入和附件保留供修改或重发；不增加完成检测器或自动重试模型。
