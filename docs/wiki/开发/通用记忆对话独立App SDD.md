...
模式选择器是工具权限的唯一来源，用户消息中的文字不得开启或关闭工具。快速模式基于当前上下文和模型自身已有的知识回答，不得调用任何工具访问项目资料。记忆模式提供完整候选工具，模型按任务自主决定是否调用。不支持工具调用的模型可以用于快速模式，但不能进入记忆模式伪装成具有 Skill 能力。

### 7.3 上下文

- Raw 永远保存已经完成的完整问答轮次，不保存失败发送尝试。
- 两种模式都把当前 Raw 中已完成的用户/助手轮次作为当前对话上下文；这是普通模型消息，不是工具调用。
- 记忆模式在同一上下文上额外提供当前 Raw 路径和完整候选工具；快速模式不提供任何工具、附件或项目资料。
...
3. 快速模式的请求不包含任何工具、附件、项目引用或网址读取能力；记忆模式在同一上下文上提供当前 Raw 路径和完整候选工具。
