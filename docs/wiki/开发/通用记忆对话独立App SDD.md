...
- 中央顶栏使用纯文字“新建对话”、分组模型选择器和设置按钮；模型选择器不使用系统原生长列表，按模型家族/厂商分组，在固定宽度浮层内滚动，保持网关原始顺序和现有 `setModel()` 合同。
...
11. “新建对话”不显示装饰图标；模型列表按 Claude、GPT、Gemini、Grok、DeepSeek、通义、智谱、RunningHub、本地模型和其他分组，浮层尺寸不随单个模型名称变化。
