...
```text
用户点击发送
  -> App 从当前对话 Raw 解析已完成轮次，与本轮用户消息组成当前对话上下文，不写本轮 Raw
  -> 快速：只发送当前对话上下文，不提供任何工具、附件、项目引用或网址读取能力
  -> 记忆：发送同一当前对话上下文和本轮附件，并提供当前 Raw 路径与 Wiki/Skill/项目/MCP 工具
...
App 只把当前对话 Raw 中已完成的用户/助手轮次解析为普通消息上下文，不做语义检索，不预查 Wiki，不把固定查询结果拼进请求。最新用户消息只出现一次。失败、取消或流式中断时 Raw 不写半轮，输入和附件仍可修改或重发。
...
模式选择器是工具权限的唯一来源，用户消息中的文字不得开启或关闭工具。快速模式基于当前上下文、模型自身已有的知识和按需只读 Wiki 查询回答；只允许调用 `wiki_search`，不得调用其他工具或访问 Wiki 之外的项目资料。记忆模式提供完整候选工具，模型按任务自主决定是否调用。两种模式都需要当前模型支持工具调用。

### 7.3 上下文

- Raw 永远保存已经完成的完整问答轮次，不保存失败发送尝试。
- 两种模式都把当前 Raw 中已完成的用户/助手轮次作为当前对话上下文；这是普通模型消息，不是工具调用。
...
- 两条路径共享当前 Raw 解析出的对话上下文；快速路径只使用该上下文和模型自身知识，记忆路径额外提供当前 Raw、Skill、项目和 MCP 工具。
- “快”来自完全不提供工具、附件、项目引用和网址读取能力，不是来自丢弃当前对话上下文。
...
2. 两种模式都把当前 Raw 中已完成的用户/助手轮次和本轮用户消息作为当前对话上下文；完整回复成功后，本轮用户消息与助手回复才以一个完整轮次写入 Raw。
3. 快速模式的请求只包含只读 `wiki_search`，不包含附件、项目引用、网址读取或其他工具；记忆模式在同一上下文上提供当前 Raw 路径和完整候选工具。
4. 模式选择器是工具权限的唯一来源，用户消息中的文字不能改变工具池。
