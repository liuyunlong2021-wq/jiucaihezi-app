...
> 日期：2026-07-25
> 状态：Web 本地底座完成；按“Mac 私测 → Web 预览/正式发布 → Mobile”推进
...
文件树就是唯一导航，也是对话选择器；不再增加项目选择器、对话下拉菜单、应用级模式导航、编辑区、创作面板或第三栏设置区。输入框上方只保留控制下一次发送的 `快速 | 记忆` 两段式开关。
...
文件树只负责资源导航和文件动作，不承担创建对话。记忆空间初始化后，中央区顶栏始终显示“新建对话”；没有项目时，中央区显示选择项目文件夹的空状态。
...
验证：本地 Web 用户无需模式、项目选择器或对话选择器，只用文件树即可打开文档/媒体、切换对话、查询和写入 Wiki，并完成公共媒体计划验证。
...
- 记忆模式文件树顶栏固定为新建文件、新建文件夹、切换项目、刷新和隐藏文件树五个动作；中央区顶栏常驻“新建对话”，切换项目会打开目标项目已有的首条 Raw，没有对话时显示显式创建入口。
...
- 新工作台顶栏复用产品 Logo，并恢复 Tauri 原生窗口拖拽区域。
...
- 中央顶栏使用纯文字“新建对话”、分组文本模型选择器和设置按钮；触发框宽度跟随当前模型名称并设响应式上限，浮层与触发框左对齐。列表按模型家族/厂商分组，保持网关原始顺序和现有 `setModel()` 合同；RunningHub 等内部媒体模型不进入文本模型选择器。
...
8. 文件树顶栏只有新建文件、新建文件夹、切换项目、刷新和隐藏文件树五个动作；“新建对话”位于中央区顶栏。
