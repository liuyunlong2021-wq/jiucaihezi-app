...
> 日期：2026-07-28
> 状态：V2.1.0 Web 与桌面正式版已发布；第四阶段 4A、4B、4C-1 至 4C-13 已完成，下一阶段为 Android 构建与真机适配。
...
### 14.29 Android 4D-2 真机安装与稳定启动结果（2026-07-28）

- 设备：vivo `V2443BA`，Android `15`，API `35`，ADB 序列号 `10AF8X1HSZ00164`；USB 调试及 USB 安装授权已由用户在设备端确认。
- 安装结果：Debug APK 已通过系统安装器安装，包名 `com.jiucaihezi.desktop`，`versionName=2.1.0`、`versionCode=2001000`。
...
### 14.31 Android 4D-3 真机会话验收结果（2026-07-28）

- 真账号登录：用户在 vivo `V2443BA` 上完成登录并勾选“保持登录”；设置页显示“已登录，可直接使用”。
- 杀进程恢复：`adb shell am force-stop` 后冷启动，设置页仍显示“已登录，可直接使用”，证明 Session 从 Android Keystore 恢复而非只依赖内存。
- 退出与失效：通过真实设置页点击“退出登录”，界面显示“已退出登录”；随后再次杀进程重开只出现“一键登录”，旧 Session 没有从 Keystore 恢复。该动作同时调用现有 Gateway logout 合同并清除本地安全凭据。
