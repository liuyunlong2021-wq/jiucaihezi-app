...
> 日期：2026-07-27
> 状态：Web 与 Mac 已完成；第四阶段 4A 云项目入口已完成，真机验收后建立 Mobile App
...
项目入口显示云端状态：`未上传 / 正在下载 / 已同步 / 待同步 N 项 / 同步失败 / 存在冲突`。设置页只保留账号、同步总开关、状态和诊断，不承担项目配对。旧的“先绑定本地文件夹，再在设置中连接已有云项目”交互已移除。
