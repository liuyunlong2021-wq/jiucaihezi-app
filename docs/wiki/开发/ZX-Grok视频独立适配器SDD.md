...
> 日期：2026-08-01
> 状态：历史适配方案；当前生产三个 Grok 渠道已直连 `https://img-api.zxcode.vip`，仓库适配代码保留但未被该渠道使用
> 范围：为 ZX `grok-1.5-video-6s/10s/15s` 提供协议转换；不修改 NewAPI 源码，不影响 RunningHub、Jina 或其他渠道。

> 2026-08-19 现行更正：用户提供的 NewAPI 后台截图显示，三个 Grok 模型所在渠道 Base URL 为 `https://img-api.zxcode.vip`。因此下文“目标架构”和部署记录属于历史方案，不代表当前生产路由；现行路由见 [[ZX视频适配器多模型升级TDD-2026-08-18#ZX 视频最终路由表]]。
