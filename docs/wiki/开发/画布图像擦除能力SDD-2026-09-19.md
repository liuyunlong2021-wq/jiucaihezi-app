...
> 状态：**桌面端已实施**（2026-09-19）。算法、命令、画布交互与落盘链路已接入并通过自动化测试；待人工在 App 内验收。
...
| 落盘 | ✅ 已实施 | 复用 `writeProjectMedia` + `addMediaToCanvas`，结果作为新图片进画布 | 待人工验收 |
| 魔棒选区 | ❌ 未做 | —— | —— |
| 软边/羽化 | ❌ 未做（当前是硬边矩形） | —— | —— |
| 显式还原入口 | ❌ 未做（原图本就未被改动） | —— | —— |
...
- App 内人工验收：选中图片 → 框选 → 擦除 → 新图进画布，这条真实链路没人跑过。
