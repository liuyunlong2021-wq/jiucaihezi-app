...
## [2026-07-26] 修正 | 记忆工作台媒体结果 OOM

- Chrome 17:54、17:58 两份 Renderer dump 明确记录 V8 OOM，且包含当前 `gpt-image-2-vip` 媒体任务；生成 HTTP 200 和项目落盘均已完成，故根因在结果展示而非模型或网关。
- 删除 `MediaTaskBubble` 挂载时的 OPFS 整文件读取、额外 `Uint8Array` 复制与 Blob URL；卡片直接懒加载受信 HTTPS 结果，视频和音频只预取元数据。
- 项目副本、下载、文件树预览和历史引用继续保留，但只在用户主动操作时读取二进制。新增防回退合同，禁止生命周期自动搬运项目媒体。

## [2026-07-26] SDD 决策 | 对话导航与文件树职责分离

- 更新《通用记忆对话独立 App SDD》：顶部轻量对话选择器负责 Raw 对话切换、创建、重命名和删除；文件树只负责 Wiki 与项目资料。
- `.raw/对话记录/` 默认隐藏为系统目录，选择器实时读取同一批 Raw，不建立第二份对话存储或索引。
- 普通文件统一进入临时预览层；返回、Esc 或关闭后恢复原对话、滚动位置、输入草稿和附件。

## [2026-07-26] 模型展示 | GPT Image 2 分组价格

- `gpt-image-2` 的 APP 展示价不再写死为错误的 `¥0.15`，改为“分组计价 · 自动账户 ¥0.12”；实际扣费继续由 NewAPI 按 Key 所属分组执行。
- `gpt-image-2-vip` 补齐固定展示价 `¥0.20`；同步新手指南，不修改 NewAPI、渠道或执行合同。

## [2026-07-26] 媒体修正 | 正式媒体结果信任执行来源

- Web 的 `gpt-image-2` 已成功生成，但上游 CDN 域名变化被客户端静态白名单误判为失败；根因是信任绑定了结果域名，而不是正式媒体执行链。
- `mediaTaskStore` 正式执行结果改为只做 HTTPS、公网地址和无账号密码的基础安全检查，不再维护上游 CDN 域名表；普通聊天、用户输入和 Skill 输出仍不进入该可信链。
- Web 媒体下载连接允许 HTTPS，并在任务结果卡保留“原始链接”复制兜底；Desktop 本轮不发布、不扩大 CSP。
...
## [2026-07-25] 修正 | 记忆工作台模型边界与独立滚动

- 文本模型选择器过滤 `rh-*`、`*/rh-*` 和 RunningHub 内部媒体模型；媒体生成能力仍由对话内公共媒体合同调用，不暴露成文本模型分组。
- 模型触发框改为内容宽度并保留响应式上限，展开浮层与其左边缘对齐。
- 文件树和文档预览补齐 `min-height: 0` 与独立原生竖向滚动，显示稳定滚动条，鼠标滚轮不再被外层布局吞掉。
- 验证通过：记忆工作台定向回归 `19/19`、`vue-tsc -b`、Web 正式构建/产物审计和 Apple Silicon Mac `.app` / DMG 签名构建；本地页面实测模型触发框与浮层左边缘相等，菜单不含 RunningHub。

## [2026-07-25] 修正 | 记忆工作台媒体结果交付

- Web 与 Desktop 成功媒体都保留 `jc-media/...` 项目相对路径；记忆 Raw 记录项目路径而不是临时 CDN URL。
...
## [2026-07-26] 实施 | 记忆工作台对话导航与统一预览

- 顶部新增对话选择器，直接投影 `.raw/对话记录/`，支持搜索、切换、新建、重命名和可恢复删除；Raw 仍是唯一真源，没有新增会话数据库或索引。
- 记忆模式文件树默认隐藏整个 `.raw`，只负责 Wiki、媒体和项目资料。
- 普通文档和媒体改为覆盖当前对话的临时预览层，提供“返回对话”、关闭与 Esc；预览不替换对话组件，因此原滚动位置、输入草稿、附件和引用保持不变。
- 验证通过：记忆工作台定向回归 `21/21`、`vue-tsc -b`、Web quick build与产物审计；应用内浏览器实测桌面和 390px 窄屏布局、Raw 隐藏、对话列表、文档预览和草稿恢复。

## [2026-07-26] 优化 | 记忆工作台顶部职责分组

- 文件树头部改为两排：第一排组合 Logo、当前项目名/切换入口和隐藏按钮，第二排只保留新建文件、新建文件夹与刷新；项目身份不再占用中央对话区。
- 中央顶栏把对话选择器与纯文字“新建对话”紧邻成组，模型和设置继续靠右，中间空白仍是 Desktop 原生拖拽区域。
- 验证通过：记忆工作台定向回归 `21/21`、TypeScript、Web quick build与产物审计；应用内浏览器实测桌面、390px 顶栏和移动文件树抽屉均无溢出或重叠。

## [2026-07-26] 修正 | 批量媒体计划恢复确认卡

- 生产反馈中，模型把三张参考图规划成一个 `jc-media-plan` 对象数组；旧解析器只接受单对象并静默吞掉解析错误，导致确认卡消失、机器 JSON 裸显。
- 共享解析层新增对象数组解析和技术块剥离；记忆工作台逐项物化并显示确认卡，每张卡独立确认、提交和展示任务进度。单对象合同及主 APP 单卡路径保持不变。
- 非法计划不会被隐藏；只有成功生成确认卡后才从可见正文移除技术 JSON。
- 验证通过：真实三视频计划固件、记忆工作台与媒体桥定向回归 `49/49`、TypeScript 和 Web quick build/产物审计。

## [2026-07-26] 优化 | 记忆工作台顶部视觉对齐

- 文件树项目身份区与中央顶栏统一为 `74px` 主头部，主分界线精确贯通左右两区；三项文件动作移到主线下方，搜索框底线降为弱辅助线。
- Logo 下沉 `2px` 并把标题间距由 `7px` 收至 `5px`。浏览器实测项目标题中心 `36.5px`、Logo 光学中心 `38.5px`，符合光学修正目标。
- 记忆工作台定向回归 `21/21`、TypeScript、Web quick build与产物审计通过；桌面和 `390x844` 窄屏实测无溢出或重叠。

## [2026-07-26] 复盘 | 记忆工作台第一阶段经验固化

- 在 [[开发/通用记忆对话独立App SDD]] 新增“已验证的产品与工程经验”，把本阶段真实反馈提炼为八条长期约束：文件单一真源、逐轮执行深度、导航职责分离、AI 输出防御性解析、素材引用物化、失败终态、视觉基线和发布证据分层。
- 本次不另建经验文档，避免设计规则与实施 SDD 分叉；后续独立工作台可以复用原则，但业务 Wiki 模板和流程仍各自定义。
# 2026-07-26 通用记忆工作台媒体稳定性与变体生成

- 根因：Veo 等 NewAPI 直连任务已完成，但结果地址 `/v1/videos/{task}/content` 仍需 Token；Web 缓存下载未携权，401 被错误合并为“生成失败”。
- 修复：仅向 `api.jiucaihezi.studio` 的媒体结果请求附加当前 Token；第三方 HTTPS 结果不携带凭据。生成成功与项目保存失败分层，保存失败保留结果并允许重试。
- 体验：确认卡提交后仍可调整模型、比例、分辨率和时长再次生成；图片支持一次提交 1-5 个独立任务，同一卡保留全部任务与结果，部分提交失败不隐藏已提交任务。

## [2026-07-26] 修正 | 记忆工作台历史上传附件可复用

- 根因：完整 Raw 只保存了附件名称、类型和大小，附件本体仅存在首次上传页面的 Data URL；后续轮和页面刷新后无法向媒体计划恢复原图，换模型再生成会退化为文生媒体。
- 修复：上传或粘贴附件时按需写入当前项目 `jc-media/uploads/`，Raw 保存受限的项目相对路径；每轮从当前对话重建历史图片/视频的受控引用，当前轮附件自动选中，历史附件由模型通过 `referenceIds` 明确选择。
- 确认卡和付费提交均从项目文件重新读取选中素材，切换模型、比例、分辨率、时长或重开对话不再清空引用；多候选且指代不明时要求澄清，不静默猜图。
- 验证通过：TypeScript、附件 Raw 与记忆工作台、公共媒体引用和提交桥专项 `52/52`、Web quick build与产物审计；应用内浏览器实测上传后文件树立即出现 `jc-media` 且缩略图正常，无控制台错误。

## [2026-07-26] 修正 | Desktop 历史参考图 MIME 恢复

- Mac 2.0.1 用已落盘参考图再次生成时，Desktop 二进制读取没有返回 MIME，公共数据 URL 退化为 `application/octet-stream`，RH 因不支持该附件类型返回 400；NewAPI “渠道不可用”是截图中的另一条独立错误。
- 共享项目媒体读取现在按“二进制 MIME -> 资源 MIME -> 受控文件扩展名”恢复图片、视频和音频类型，不修改 Seedance、RH adapter 或渠道合同。
- 回归先复现 `data:application/octet-stream`，修复后媒体引用、确认卡、提交桥和记忆工作台专项 `56/56`、TypeScript 与变更文件 lint 通过。

## [2026-07-26] 修正 | 媒体提示词控制权与同名模型消歧

- 媒体规则改为：用户明确给出的提示词或动作描述默认原样使用，只有用户要求优化或没有可执行描述时才允许模型补全。
...
## [2026-07-27] 实施 | 云项目正式产品入口

- 左上项目名升级为唯一项目中心，统一显示当前项目、本机项目、云端项目、新建项目和 Desktop 本地文件夹入口；设置页移除云项目配对控件，只保留状态和“立即同步”。
- 本地项目可首次“上传到云端”；选择云项目时优先复用已绑定的本地副本，否则 Web 创建空浏览器项目，Mac 要求选择空文件夹，再先拉取云端文字，避免模板文件抢先上传制造冲突。
- 首次绑定后仍沿用既有本地优先队列：文本成功写入自动上传，项目打开、App 回到前台和网络恢复自动先拉后推，手动入口只负责强制执行/失败重试。媒体、Skill、MCP 和凭据仍不同步。
- 完整 focused 回归 `1349/1349`、Rust `397/397`、TypeScript、Web/Desktop 正式构建与产物审计全部通过；本地 Web 项目中心视觉与交互验收通过，无控制台错误。

## [2026-07-27] 修正 | 云同步登录态启动恢复

- v2.0.4 真机发现普通 API Key 已恢复但独立 `sync_session` 未初始化，账号页因此显示已登录，而项目中心和同步页仍按空内存会话拒绝请求。
- 启动现同时恢复 API Key 与同步会话；统一响应式同步鉴权状态驱动 Web/Mac 项目中心和设置页，上传动作在鉴权判断前主动等待持久化会话恢复。只有旧 API Key、没有同步会话的用户会被明确要求重新登录一次。
- APP 截图中的旧文件夹选择器确认来自 `/Volumes/韭菜盒子 1` 内仍运行的 `2.0.3`；`/Applications/韭菜盒子.app` 已是 `2.0.4`。专项回归 `62/62`、TypeScript、Web/Desktop quick build与产物审计通过，浏览器确认项目入口打开统一项目中心。
## 2026-07-27 — 云项目真机闭环与首次同步进度

- v2.0.4 Mac 重新登录后上传本地项目，Web 同账号成功下载完整 Raw/Wiki 文字副本；确认 Wiki 未被过滤，媒体与空目录按合同不进入同步。
- 客户端首次上传从单 mutation 请求改为每批最多 100 个不同路径；保留同路径 revision 顺序和原冲突/幂等合同。
- 项目中心与设置页共用扫描、上传、下载的完成数/总数和原生进度条，并明确“只同步文字，媒体和空目录不同步”。
