...
```curl
curl --location --request POST 'https://www.runninghub.cn/openapi/v2/youchuan/text-to-image-v81' \
--header "Content-Type: application/json" \
--header "Authorization: Bearer ${RUNNINGHUB_API_KEY}" \
--data-raw '{
  "prompt": "原木桌面轻奢咖啡器具组合，陶瓷咖啡杯配银质小勺，袅袅热气，窗边柔和漫射自然光，浅景深虚化，实木与陶瓷细腻材质纹理，暖调极简静物摄影，高清微距细节",
  "chaos": 0,
  "quality": "1",
  "stylize": 0,
  "raw": false,
...
| --- | --- | --- | --- |
| `prompt` | String | 必填 | 文本长度限制: 1 - 8192 |
| `chaos` | Int | 可选 | 混沌参数，为图像结果增添趣味 (0-100)<br>输入范围值: 0 - 100 |
| `quality` | String | 可选 | 图像质量<br>枚举值: [1, 4] |
| `stylize` | Int | 可选 | 风格化参数，控制图像的艺术风格强度（0-1000）<br>输入范围值: 0 - 1000 |
| `raw` | Boolean | 可选 | 原始模式，获得对图像的更多控制 |
| `imageUrl` | String | 可选 | 输入图片URL（垫图）<br>最多支持 1 项图片，每张 20 MB |
| `iw` | Int | 可选 | 图像权重，控制图像提示的影响<br>输入范围值: 0 - 3 |
| `sref` | String | 可选 | 风格参考图片URL<br>最多支持 1 项图片，每张 20 MB |
| `sw` | Int | 可选 | 风格权重，控制风格参考的影响（0-1000，需搭配sref使用）<br>输入范围值: 0 - 1000 |
| `sv` | Int | 可选 | 风格版本<br>输入范围值: 6 - 6 |
| `aspectRatio` | String | 可选 | 枚举值: [1:1, 4:3, 3:2, 16:9, 3:4, 2:3, 9:16] |
| `hd` | Boolean | 必填 | 是否开启原生 2K |
...
```curl
curl --location --request POST 'https://www.runninghub.cn/openapi/v2/rhart-image-g/text-to-image' \
--header "Content-Type: application/json" \
--header "Authorization: Bearer ${RUNNINGHUB_API_KEY}" \
--data-raw '{
  "model": "g-4.2",
...
| 参数说明 | 类型 | 必填/可选 | AI 应用程序生成的结果。 |
| --- | --- | --- | --- |
| `model` | String | 必填 | 枚举值: [g-3, g-4, g-4.1, g-4.2] |
| `prompt` | String | 必填 |  |
| `aspectRatio` | String | 可选 | 枚举值: [960x960, 720x1280, 1280x720, 1168x784, 784x1168] |
...
```curl
curl --location --request POST 'https://www.runninghub.cn/openapi/v2/rhart-image-g/image-to-image' \
--header "Content-Type: application/json" \
--header "Authorization: Bearer ${RUNNINGHUB_API_KEY}" \
--data-raw '{
  "model": "g-4.2",
...
| `model` | String | 必填 | 枚举值: [g-3, g-4, g-4.1, g-4.2] |
| `prompt` | String | 必填 |  |
| `imageUrl` | String | 可选 | 最多支持 1 项图片，每张 10 MB |
