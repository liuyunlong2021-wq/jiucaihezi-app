...
核查结论（下表为**迁移前**快照 + v3.1 已执行的动作；「实际」列指 2026-07-01 迁移动手之前的状态，现均已按末列做实）：

| 创作 skill | 交接文档声称 | 迁移前实际 | v3.1 动作（已完成） |
|-----------|:-----------:|------|----------|
| JC-duanju-shijiemoxing | 已对齐 | ✅ 真的对齐 `wiki/`（但运行时未安装） | 补 log.md + 补装运行时 |
