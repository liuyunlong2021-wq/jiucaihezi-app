...
- 右上角有两个独立开关：“对话记忆”控制回答完成后的自动索引，“对话查询”控制下一轮是否预取当前对话的 `memory_search`；新建对话默认都开启，互不影响。
...
- **对话记忆**：开启后，成功回答在 Raw 保存完成后自动生成 `summary + keywords` 并记录到当前对话索引；关闭只停止后续自动记录，不删除已有 Raw 或索引。
- **对话查询**：开启后，下一轮发送前自动查询当前对话的索引；关闭不查询历史索引，但最近三轮工作记忆仍然保留。
