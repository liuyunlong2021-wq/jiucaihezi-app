...
不归这个脚本管——那要跟用户确认后手动编辑或转交 JC-jiyiyasuo，机械脚本
不该替用户决定内容。
...
def cmd_scaffold(args):
    ...
    if os.path.isdir(category_dir):
        print(f"⚠️  {category_dir} 已存在，不会覆盖。如需扩充内容请直接编辑或转交 JC-jiyiyasuo。")
        sys.exit(1)
    ...
    if args.apply:
        ...
        print("-" * 56)
        print("✅ 已创建。")
        print(f"下一步：如果这个分类需要吃掉已有内容（从别处迁移/重新归档），转交 JC-jiyiyasuo 处理；")
