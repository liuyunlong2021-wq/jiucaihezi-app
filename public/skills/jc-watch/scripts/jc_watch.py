#!/usr/bin/env python3
"""jc-watch entry point: download a video, extract frames, parse the transcript.

Prints a markdown report to stdout listing frame paths + transcript. The agent
then reads each frame path as an image to actually *see* the video.

Ported from bradautomates/claude-video (MIT). Two changes: transcription runs on
the machine's own whisper-cli instead of the Groq/OpenAI Whisper API, and caption
selection is language-aware (Chinese first) instead of English-only.
"""
...
from local_whisper import load_api_key, missing_reason, transcribe_video  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument(
        "--no-whisper",
        action="store_true",
        help="Disable the local whisper fallback. Report frames-only if no captions available.",
    )
    ap.add_argument(
        "--whisper-model",
        type=str,
        default=None,
        help="Path to a whisper.cpp ggml model file. Default: auto-detect from the usual cache dirs.",
    )
    ap.add_argument(
        "--language",
        type=str,
        default="auto",
        help="Spoken language for transcription: auto (default), zh, en, …",
    )
    ap.add_argument(
        "--no-dedup",
        action="store_true",
    ...
    if args.out_dir:
        ...
        work = Path(tempfile.mkdtemp(prefix="jc-watch-"))
    work.mkdir(parents=True, exist_ok=True)
    print(f"[jc-watch] working dir: {work}", file=sys.stderr)
    ...
    if url_source:
        print("[jc-watch] checking metadata/captions via yt-dlp…", file=sys.stderr)
        ...
        if dl.get("subtitle_path"):
            try:
                ...
                print(f"[jc-watch] subtitle parse failed: {exc}", file=sys.stderr)
                transcript_segments = []
    ...
    if detail == "transcript" and transcript_segments and not cue_timestamps:
        ...
        if url_source:
            print(
                "[jc-watch] downloading audio via yt-dlp…" if audio_only
                else "[jc-watch] downloading video via yt-dlp…",
                file=sys.stderr,
            )
            dl = download(
                args.source,
            ...
            print("[jc-watch] using local file…", file=sys.stderr)
    ...
    if cue_timestamps and video_path:
        ...
        if cue_meta.get("dropped_out_of_window"):
            print(
                f"[jc-watch] {cue_meta['dropped_out_of_window']} cue timestamp(s) outside the "
                "focus range — dropped",
                file=sys.stderr,
            )
    ...
    if detail != "transcript" and video_path and detail_budget != 0:
        ...
        print(
            f"[jc-watch] extracting {engine_label} over {scope} "
    ...
    if not transcript_segments and not args.no_whisper and video_path and meta.get("has_audio"):
        ...
        if backend and model_path:
            try:
                ...
                whisper_note = str(exc)
                print(f"[jc-watch] local whisper failed: {exc}", file=sys.stderr)
        else:
            whisper_note = missing_reason()
            print(f"[jc-watch] local whisper unavailable — {whisper_note}", file=sys.stderr)
    elif not transcript_segments and video_path and not meta.get("has_audio"):
        print("[jc-watch] no audio stream found — proceeding without transcription", file=sys.stderr)

    info = dl.get("info") or {}

    print()
    print("# jc-watch: video report")
    print()
    ...
    if transcript_text:
        ...
        print(
            "_No transcript available at transcript detail. Captions were missing and local whisper "
            "was unavailable or failed, so there is no visual fallback here. Re-run with "
            "`--detail balanced` for frames._"
        )
    elif focused and dl.get("subtitle_path"):
        print(f"_No transcript lines fell inside {format_time(effective_start)} → {format_time(effective_end)}._")
    else:
        reason = whisper_note or missing_reason()
        print(
            f"_No transcript available — proceed with frames only. Local whisper could not run: {reason}_"
        )
