---
name: JC-影视风格定调
description: 项目级风格决策。输入梗概→定媒介/比例/导演+作品→Pinterest搜色卡→输出 style-design.md。一部剧定一次，下游复用。
triggers:
  - "风格定调"
  - "影视风格"
  - "选导演"
...
以下 Skill 读取本输出：
- **JC-剧本转分镜**：读导演+作品 → 生成节奏档案；读色卡库 → 逐场选色卡
- **JC-角色设计提示词**：读媒介+导演风格 → 角色视觉DNA
- **JC-场景提示词**：读色卡库 → 场景配色约束
