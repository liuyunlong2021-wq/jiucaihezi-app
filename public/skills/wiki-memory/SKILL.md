...
- 沉淀故事、小说、新闻故事或剧本时，必须读取并遵守 [故事资料沉淀规则](references/故事资料沉淀规则.md)。沉淀技术书、论文、规范标准、内部文档或方法论文集这类非叙事物料时，必须读取并遵守 [知识资料沉淀规则](references/知识资料沉淀规则.md)——它复用了同一套两阶段原则，只在语义字段、横向页和提交链路上不同。其他资料类型没有专用规则时，沿用上述两阶段原则，不能套用故事资产类型。
...
6. 回执里每次提到项目内文件都必须写成双链 `[[<完整项目路径>|<显示名称>]]`；不要写普通 Markdown 链接、裸文件名或工作目录相对路径。只有双链才能让用户点开真实文件并定位到文件树，其他写法会让链接失效。
