---
name: JC-manju-changjing
description: Use when turning script locations into a final empty-environment master-shot scene asset for the current Banana/Veo short-drama pipeline. This stage must inherit the selected type-analysis style and lighting, lock one reusable camera truth, exhaustively enumerate everything visible in that master shot, and output Banana Pro JSON prompts for image generation.
triggers:
  - "场景分析"
  - "场景设定"
  - "场景资产"
...
- `prompts/banana-scene-prompts.json` — Banana Pro JSON prompts for scene image generation
...
11. `banana_prompts` inherits `ratio_design` and `style_design` from `control_table.style_inheritance`.

---

## Banana Pro Scene Prompt Generation

This section converts `control_table` fields into machine-readable Banana Pro JSON prompts for scene asset image generation.

### Input

- `analysis/scene-analysis.json` (the `control_table` fields)
- `analysis/film-type-analysis.json` (for `selected_option`)
  ...
  "scenes": [
    {
      ...
      "prompt": {
        ...
        "human_presence_rule": "empty environment plate, no visible people, no human figure, no character",
        "master_shot_view": "终极master shot视角描述",
        "foreground_midground_background": "前中后景的关键元素分层",
        "key_visual_elements": "关键环境锚点",
...
|---|---|
| `scene_identity_sentence` | `scene_narrative_identity` |
| `human_presence_rule` | `human_presence_policy.mode` → `no_people` or `background_crowd_only` |
| `master_shot_view` | `master_shot_camera_design` + `master_shot_subject_blocking` |
| `foreground_midground_background` | `master_shot_environment_manifest.foreground/midground/background` |
| `key_visual_elements` | `master_shot_environment_manifest.visible_environment_details` — decisive anchors only, not exhaustive |
| `spatial_type` | Infer from scene: `封闭空间` (interior room), `开放空间` (exterior), `过渡空间` (corridor/hallway) |
| `lighting` | `master_shot_lighting_design` — exact inheritance, no generic substitution |
| `shot_language` | `master_shot_camera_design.shot_size` + `composition_logic` |
| `aspect_ratio` | `style_inheritance.ratio_design` — do not silently switch portrait/landscape |

### Writing Rules

1. **scene_identity_sentence first**: Highest-weight opening. States whose space, what dramatic use, what narrative identity. Example: "A cramped bachelor bedroom where a half-awake young man is cornered by an unexpected mother-and-child intrusion."

2. **human_presence_rule explicit**: Default `empty environment plate, no visible people`. Only allow `background crowd only, no singled-out hero person, crowd acts as environmental mass` when crowd is truly environmental.

3. **master_shot_view = camera truth**: What this view covers, where key furniture/doors/windows are, what confrontation corridor remains visible.

4. **lighting inherits exactly**: From `master_shot_lighting_design`. Do not replace with generic studio light. Key source, fill, practicals, color temp, contrast, exposure.

5. **key_visual_elements selective not exhaustive**: Prioritize bed/mattress/bedding, door/handle/frame, window/curtains, desk/chair, wardrobe, floor/ceiling, visible clutter. Let model complete minor implied detail.

6. **shot_language = director-level**: Shot size, camera height/side, composition logic, zone names from `scene_zone_map`.

7. **technical_specs fixed**: `8K resolution, photorealistic PBR textures, environment concept art, single-point perspective, zero lens distortion`.

8. **negative_prompt required**: text, watermark, stylization, wrong perspective, spatial mismatch, lighting inconsistency, overexposure, complex distortion.
