---
name: JC-manju-fenjing
description: Use when converting script content and engineering-book material units into executable shot design for xiaolagumanju. This skill must inherit pacing, respect the material engineering book, output downstream-useful shot control data, 3×3 storyboard grid prompts, and video generation prompts (Grok/Veo/LTX).
triggers:
  - "镜头设计"
  - "分镜"
  - "shot design"
...
- `prompts/banana-grid-shot-prompts.json` — 3×3 storyboard grid batch prompts
- `prompts/grok-video-prompts.json` — Grok image-to-video prompts
- `prompts/veo-video-prompts.json` — Veo image-to-video / start-end-to-video prompts
- `prompts/ltx-video-prompts.json` — LTX 2.3 video action prompts
...
10. Output 3×3 storyboard grid prompt batches for every scene.
11. Output video prompts for every shot (Grok/Veo/LTX format).

---

## 3×3 Storyboard Grid Generation

Converts `shot-design.json` shots into machine-readable 3×3 storyboard grid prompt batches.

### Core Rules

1. **Assembly only**: Do not invent new shots, staging, emotions, or camera grammar not in shot-design.
2. **3×3 default**: Pack slots into exact 3×3 batches. `grid_aspect_ratio` inherits project ratio from `ratio_design`.
3. **Short beats**: Each `prompt_text` ≤ 35 English words. Read like a tiny cinematic action beat, not a rigid tag chain.
4. **Video routing**: `image_to_video` shots → one `first_frame` slot. `start_end_to_video` shots → two consecutive slots (`first_frame` + `last_frame`).
5. **No empty panels**: If the final batch is short, add a `continuity_hold` slot derived from an existing shot state — never invent new action.

### Global Constraint (every batch)
  ...
  "batches": [
    {
      ...
      "global_constraint": "Borderless seamless grid...",
      "reference_images": ["path/to/char.png", "path/to/scene.png"],
      "slots": [
        {
          "slot_index": 1,
          "source_shot_id": "shot_001",
          "slot_type": "first_frame",
          "prompt_text": "[shot size] [action beat] [dominant subject]"
        }
      ]
    }
  ],
  "frame_registry": [
    {
      "shot_id": "shot_001",
      "video_generation_mode": "image_to_video",
      "first_frame": { "batch_id": "batch_001", "panel_index": 1 }
    }
  ]
}
```
...
Converts shot-design into video prompts for each supported endpoint. Assembly-only — no new actions, dialogue, or camera moves.

### Grok Video (image-to-video only)

Target: `rhart-video-g/image-to-video`

Hard limits: `duration`: 6s/10s/15s, `aspectRatio`: 2:3/3:2/1:1, `resolution`: 720P/1080P.
Default for portrait storyboard: `aspectRatio: 2:3`, `resolution: 720P`, `duration: 6s`.

Prompt format (Chinese segmented timeline):

```
[shot size], [action beat], [subject]. cut. [next beat]. 无音乐，无音效，无字幕。
```

### Veo Video (image-to-video + start-end)

Mode selection from `shot-design.json`:
- `video_generation_mode: image_to_video` → first-frame mode, one input image
- `video_generation_mode: start_end_to_video` → start-end mode, two input images (first + last frame)

Prompt: short English cinematic beat describing motion between frames. Preserve `StyleDesign` in atmosphere cues.

### LTX 2.3 Video Action

Uses compact shot-design fields: `first_frame_cn`, `last_frame_cn`, `shot_prompt_cn`, `dialogue`, `duration_seconds`.

**Prompt structure** (one English paragraph):
`[Setup] + [Action Description] + [Camera] + [Audio]`

**Dialogue rule**: If Chinese dialogue exists, format as:
`用标准普通话说：The character says, 中文台词 in a [Tone] voice.`

If no dialogue: no `says`/`speaks`/`voice` in prompt.

**Duration**: `final_duration = max(base_duration, ceil(dialogue_char_count / 5) + 1)`
