...
usage() {
  cat <<'EOF'
用法:
  ./install.sh                 安装到 ~/.agents/skills/JC-口播
  ./install.sh --codex         共享目录 + ~/.codex/skills/JC-口播
  ./install.sh --claude        共享目录 + ~/.claude/skills/JC-口播
...
install_shared() {
  local destination="$HOME_DIR/.agents/skills/JC-口播"
...
if has_target codex && ! has_target all; then
  ...
  install_one "$HOME_DIR/.codex/skills/JC-口播" "codex"
fi

if has_target claude && ! has_target all; then
  mkdir -p "$HOME_DIR/.claude/skills"
  install_one "$HOME_DIR/.claude/skills/JC-口播" "claude"
fi

if has_target all; then
  [[ -d "$HOME_DIR/.codex/skills" ]] && install_one "$HOME_DIR/.codex/skills/JC-口播" "codex"
  [[ -d "$HOME_DIR/.claude/skills" ]] && install_one "$HOME_DIR/.claude/skills/JC-口播" "claude"
fi
