...
当用户上传照片并要求「参考照片、用我的脸、能看出来是我」时，核心策略是风格化重绘而非照片拼贴。

### 参考图角色分配

- **身份参考**：脸型、笑容、五官比例、发型轮廓等识别锚点
- **场景参考**：山景、街道、室内等空间关系
- **穿着/道具参考**：帽子、墨镜、外套、围巾等硬约束

### 统一重绘约束

```
使用上传照片作为参考，整张图统一重绘成目标画风；人物、脸、衣服、道具和背景使用同一光照、边缘风格、材质语言和色彩分级。保留可识别身份特征，但不要照片脸贴风格化身体，不要绿幕抠图感，不要硬边缘光晕，不要人物和背景割裂。
```

### 像素风特殊规则

像素风不能保留照片级五官，用服装色块、帽子、墨镜、姿势、发型轮廓和笑容符号维持可识别度。必须写「整张图统一像素网格、同一分辨率边缘」。
