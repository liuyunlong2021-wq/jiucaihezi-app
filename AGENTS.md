...
| V7.2 T8 画布全量迁入 | ✅ 已完成 | 31 个节点 1:1 对照 T8-penguin-canvas 迁入。画布和创作面板已解锁，媒体生成统一走主 NewAPI Token。 |
| V7.x 媒体鉴权统一 | ✅ 已完成 | 创作面板和画布媒体生成不再提供独立媒体 Key / BYOK 配置，统一使用主 NewAPI Token。 |
| V7.x 8091 OCR 附件解析双端打通 | ✅ 已完成 | Web+桌面双端通用：图片→8091 PaddleOCR→文字注入LLM。Nginx CORS 修复、Tauri JSON base64 双模传输、未登录占位 token、`buildDirectLocalMessages` 注入。`volume_up` 图标 bundle 修复。 |
