...
>
> 最后更新：2026-06-30
> 当前发布基线：`v1.1.2` + **画布已移除** + **指令全覆盖（工具83条+Skill48条）** + **工具安装状态三段式检测** + **内置Skill自动播种** + **知识库内循环 Phase 1/2/2.5** + **小说/短剧临摹四件套 skill** + **项目文件树 VS Code Explorer 复刻** + **手机端适配 Phase 1** + **直连多模态贴图** + **上下文用量面板** + **Windows CSP/黑框修复** + **插件仓库（chajian 分支，2 个插件）**
>
...
- **Windows 适配修复**（2026-07-01，分支 `Windows`→`main`，v1.1.2）：
  - **问题1：assets 全部加载失败**（用户看到「韭菜盒子正在启动」然后超时）。根因：Tauri v2 在 Windows 上以 `http://tauri.localhost` 提供生产资源，CSP `script-src 'self'` 在 WebView2 上无法正确匹配。修复：CSP 所有指令显式加入 `http://tauri.localhost` `https://tauri.localhost`；窗口新增 `useHttpsScheme: true` 统一跨平台协议。
  - **问题2：启动时弹出 OpenCode 控制台黑框**。根因：`tokio::process::Command::spawn()` 未设置 `CREATE_NO_WINDOW`，OpenCode 作为控制台程序在 Windows 上自动弹出 cmd 窗口。修复：OpenCode server spawn 处添加 `#[cfg(windows)] creation_flags(0x08000000)`。
...
- **插件仓库 — 通用插件框架**（2026-07-01，分支 `chajian`，已测试，待合并）：
  ...
  - **安装**：paste-to-chat → LLM 执行。和 CLI 工具保持一致，不改 `GitHubSkillCard.install()`。
  - **通用性**：不锁定 OpenCode。任何编码工具（Codex/Claude Code/Cursor 等）的插件都能通过加一条 `githubTools.json` 条目接入。
  - **已收录插件**：
...
- **Windows 适配已做**：CSP/黑框修复已合入 v1.1.2，需真实 Windows 实测验证。后续若仍有启动问题，用 `__JC_BOOT_LOG__` 排查。
