...
>
> 最后更新：2026-06-29
> 当前发布基线：`v1.0.15` + **画布已移除** + **指令全覆盖（工具69条+Skill48条）** + **工具安装状态三段式检测** + **知识库内循环 Phase 1/2/2.5** + **小说临摹三件套 skill**
...
- **指令全覆盖 + 安装状态三段式检测**（2026-06-29，分支 `zhilingtianjia`→`main`）：
  - **工具仓库指令按钮**：`GitHubSkillCard.vue` 新增 🧠「指令」按钮，点击弹出 2 列指令卡片弹窗，点击指令直接粘贴到输入框。8 个工具共 69 条指令（yt-dlp 14 / FFmpeg 15 / pandoc 8 / ripgrep 6 / whisper 6 / Pixelle-Video 10 / so-novel 2 / ImageMagick 8），全覆盖各工具功能。
