...
>
> 双端同等重要：对共享产品能力（创作面板、编辑区、消息渲染、模型/Skill 配置等）的改动，应尽量保持桌面与 Web 一致。平台专属能力必须显式隔离：桌面专属是 Tauri + OpenCode + 本地工具；Web 专属是浏览器直连 + Web 持久化/搜索/工具层。
>
> 对话（聊天）不是完全共享的核心：
...
> **2026-06-25 画布已移除**：画布源码移至 `_canvas-archive/`。`webhuabu` 分支已废弃。
...
2. **Skill系统** — Skill就是官方 Skill：`SKILL.md` + 可选 `references/`、`scripts/`、`assets/`
...
| `src/services/newApiClient.ts` | NewAPI 客户端 + 主 Token 安全存储 | 鉴权传递、超时、流式/非流式双通道、Token 不可泄露到 localStorage |
...
| 内置Skill `SKILL_PRESETS` 已重建 | ✅ 已完成 | 官方 Skill + Superpower，全部通过 skill:// 协议加载 |
...
| V7.1 本地能力中心 | ✅ 已实现 | `src/utils/localCapabilities.ts` 能力注册表 + `LocalCapabilitySetup.vue` 首次引导弹窗 + 设置页内嵌。 |
| 8091 OCR 附件解析双端打通 | ✅ 已完成 | Web+桌面双端通用：图片→8091 PaddleOCR→文字注入LLM。 |

### ✅ APPyouhua 修复（v1.0.15）

| 问题 | 状态 | 说明 |
|------|------|------|
| 退出 APP 后 OpenCode 僵尸进程 | ✅ 已修复 | Drop try_lock 改为 3 次重试，确保子进程被 kill |
| 权限弹窗反复弹出 | ✅ 已修复 | session-scoped approved 规则集 + binary autoAccept toggle |
| 回复完成仍显示回复中 | ✅ 已修复 | 120s 看门狗 + onClose 3 次重试 + 轮询 5min 上限 |
| 模型选择器 OpenCode 模型污染 | ✅ 已修复 | 禁止 OpenCode 模型进入选择器，缓存仅存 gateway |
| Intel Mac 卡 logo | ✅ 已修复 | deep link 注册从关键路径移至后台 |
| Windows 无法启动 | ✅ 已修复 | HOME→USERPROFILE 跨平台兼容 |
| 画布系统 | ✅ 已移除 | v1.0.15 移至 _canvas-archive/，不再维护 |
| 知识库主链路 | ✅ 已移除 | 备份至 知识库备份/ |
...
### 4.3 Skill系统 — agentStore.ts + skill.ts
