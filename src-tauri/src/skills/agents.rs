...
fn agent_uses_central_root(agent: &Agent, central_root: &str) -> bool {
    normalize_agent_path(&agent.global_skills_dir) == central_root
}
...
fn central_root_from_agents(agents: &[Agent]) -> String {
    let central_path = agents
        .iter()
        .find(|agent| agent.id == "central")
        .map(|agent| agent.global_skills_dir.as_str())
        .unwrap_or("~/.agents/skills");
    normalize_agent_path(central_path)
}
...
pub async fn get_agents_impl(pool: &DbPool) -> Result<Vec<AgentWithStatus>, String> {
    ...
    let central_root = central_root_from_agents(&agents);
    Ok(agents
        .into_iter()
        .map(|agent| agent_to_with_status(agent, &central_root))
        .collect())
}
...
pub async fn add_custom_agent_impl(
    ...
    let agents = db::get_all_agents(pool).await?;
    let central_root = central_root_from_agents(&agents);
    Ok(agent_to_with_status(persisted, &central_root))
}
...
pub async fn update_custom_agent_impl(
    ...
    let agents = db::get_all_agents(pool).await?;
    let central_root = central_root_from_agents(&agents);
    Ok(agent_to_with_status(updated, &central_root))
}
...
#[tauri::command]
pub async fn get_agents(state: State<'_, SkillsAppState>) -> Result<Vec<AgentWithStatus>, String> {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_get_agents_reports_install_target_semantics() {
        let pool = setup_test_db().await;

        sqlx::query("UPDATE agents SET global_skills_dir = ? WHERE id = 'central'")
            .bind("/tmp/central-skills")
            .execute(&pool)
            .await
            .unwrap();
        sqlx::query("UPDATE agents SET global_skills_dir = ? WHERE id = 'codex'")
            ...
            .unwrap();
        sqlx::query("UPDATE agents SET global_skills_dir = ? WHERE id = 'claude-code'")
            .bind("/tmp/claude-skills")
            .execute(&pool)
            .await
            .unwrap();

        let agents = get_agents_impl(&pool).await.unwrap();
        let central = agents.iter().find(|a| a.id == "central").unwrap();
        let codex = agents.iter().find(|a| a.id == "codex").unwrap();
        let claude = agents.iter().find(|a| a.id == "claude-code").unwrap();

        assert!(central.uses_central_root);
        assert!(!central.is_install_target);
        assert!(codex.uses_central_root);
        assert!(!codex.is_install_target);
        assert!(!claude.uses_central_root);
        assert!(claude.is_install_target);
    }
