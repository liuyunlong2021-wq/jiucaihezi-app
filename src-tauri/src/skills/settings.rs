...
pub fn get_skills_database_path_impl() -> String {
    path_to_string(&app_data_dir().join("db.sqlite"))
}
...
#[tauri::command]
pub async fn get_skills_database_path() -> Result<String, String> {
    Ok(get_skills_database_path_impl())
}
...
mod tests {
    ...
    #[test]
    fn test_get_skills_database_path_returns_app_db_file() {
        let path = get_skills_database_path_impl();
        assert!(path.ends_with("db.sqlite"));
        assert!(path.contains(".skillsmanage"));
    }
}
