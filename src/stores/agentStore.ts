...
const SKILL_PRESETS: SkillConfig[] = [
  ...
  {
    id: 'preset_JC-manjuxiezuo',
    name: '漫剧逐集成稿器',
    description: '漫剧/短剧逐集成稿器。基于用户已写好的分集梗概，逐集扩写成可拍摄的成稿正文。忠于用户剧情走向，在台词/动作/镜头/反转/钩子层面发挥。触发词：分集梗概、逐集扩写、按大纲写、梗概变剧本',
    triggers: ['分集梗概', '分集大纲', '逐集', '一集集写', '扩写这一集', '按大纲写', '梗概扩写', '续写下一集', '漫剧成稿', '短剧成稿'],
    skillContent: 'skill://JC-manjuxiezuo/SKILL.md',
    source: 'preset', tier: 'L1', version: 1,
    ...PRESET_DEFAULTS,
  },
  {
    id: 'preset_JC-linmoduanju',
    name: '短剧临摹换皮器',
    description: '短剧/漫剧临摹换皮器。拿一部已写好的范本（成稿剧本，或小说/故事文），一集一集换皮改写成自己的短剧剧本——换人名/地名/时代/世界观/台词，以及范本标志性的桥段/反转设计，但保留剧情走向、人物命运、爽点节奏、付费卡点、主题。换皮不换骨，靠映射表保证全篇一致，输出标准短剧剧本格式。范本是剧本或小说都行，输出统一为剧本。触发词：临摹短剧、剧本换皮、照着这个剧本改、小说改短剧、一集一集改剧本',
    triggers: ['临摹短剧', '临摹剧本', '短剧换皮', '剧本换皮', '照着这个剧本改', '改人名写短剧', '小说改短剧', '小说临摹成短剧', '一集一集改剧本', '仿写短剧', '套用这个剧情写短剧', '改背景写剧本'],
    skillContent: 'skill://JC-linmoduanju/SKILL.md',
    source: 'preset', tier: 'L1', version: 1,
    ...PRESET_DEFAULTS,
  },
  {
    id: 'preset_JC-linmoxiaoshuo',
    name: '小说临摹换皮器',
    description: '小说临摹/换皮改写器。面向想从临摹优秀作品起步的新手：拿一部已写好的范本，一章一章换掉人名/地名/时代/世界观/台词，但保留情节走向、人物命运、主题、文风。换皮不换骨，靠"映射表"保证全篇一致，并把标志性桥段/道具/动作/符号"保功能换实现"防止一眼被认出临摹自哪部。触发词：临摹、换皮、改写小说、改人名、照着这本写、一章一章改、仿写',
    triggers: ['临摹', '换皮', '改写小说', '改人名', '改地名', '改世界观', '照着这本写', '一章一章改', '仿写', '改写这章', '套用这个故事', '改背景设定'],
    skillContent: 'skill://JC-linmoxiaoshuo/SKILL.md',
    source: 'preset', tier: 'L1', version: 1,
    ...PRESET_DEFAULTS,
  },
  {
    id: 'preset_JC-yizhixing',
    name: '知识库一致性巡检官',
    description: '知识库一致性巡检官。对 wiki 架构的小说/临摹/剧本知识库做"全库体检"，抓单章自查抓不到的跨章问题：换皮漏改、命名漂移、映射撞车、伏笔埋了没收、角色设定前后矛盾、双链断裂、孤儿笔记。只挑错不改。与逐章写作 skill 互补。触发词：巡检、一致性检查、全本检查、知识库体检、查漏改、查穿帮、前后矛盾、断链、伏笔有没有收',
    triggers: ['巡检', '一致性检查', '全本检查', '知识库体检', '查漏改', '查穿帮', '前后矛盾', '断链', '孤儿笔记', '伏笔有没有收', '设定矛盾', '换皮一致性'],
    skillContent: 'skill://JC-yizhixing/SKILL.md',
    source: 'preset', tier: 'L1', version: 1,
    ...PRESET_DEFAULTS,
  },
  {
    id: 'preset_JC-jiyiyasuo',
    name: '知识库记忆压缩官',
    description: '知识库记忆压缩官。负责"养库"：把 .raw/ 堆积的对话和素材提炼归档进 wiki/ 结构化档案，给笔记补 [[双链]]，刷新 hot.md 热缓存，让大模型续写长篇时不失忆。是长篇创作"不失忆循环"的发动机。触发词：整理记忆体、整理素材、归档raw、刷新热缓存、更新记忆小抄、整理知识库、续写前整理',
    triggers: ['整理记忆体', '整理素材', '归档raw', '刷新热缓存', '更新记忆小抄', '整理知识库', '沉淀对话', '续写前整理', '压缩记忆', '更新hot'],
    skillContent: 'skill://JC-jiyiyasuo/SKILL.md',
    source: 'preset', tier: 'L1', version: 1,
    ...PRESET_DEFAULTS,
  },
  {
