...
test('memory project paths hide selector records and protect only the fixed skeleton', () => {
  for (const path of ['.raw/对话记录', '.raw/对话记录/a.md', '.raw/记忆索引', '.raw/记忆索引/a.md', '.raw/.sync/state.json', `${MEMORY_FILE_TASK_DIRECTORY}/run-1/manifest.json`, 'jc-canvas/a.jccanvas', 'wiki/.DS_Store']) {
    assert.equal(isMemoryProjectHiddenPath(path), true, path)
  }
  for (const path of ['.raw', '.raw/jc-media', '.raw/jc-media/图片', '.raw/对话记录/a.md', '.raw/记忆索引/a.md', `${MEMORY_FILE_TASK_DIRECTORY}/run-1/manifest.json`, 'jc-canvas/a.jccanvas']) {
