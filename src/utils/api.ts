...
export function sanitizeProviderError(value: unknown, apiKey = ''): string {
  ...
  return text
    ...
    .replace(/data:[^,\s]+;base64,[A-Za-z0-9+/_=-]+(?:\r?\n[A-Za-z0-9+/_=-]+)*/gi, '[REDACTED_DATA_URL]')
    .replace(/(^|[^A-Za-z0-9+/_-])((?:[A-Za-z0-9+/]{60,}={0,2}\r?\n)+[A-Za-z0-9+/]{20,}={0,2})($|[^A-Za-z0-9+/_=-])/g, '$1[REDACTED_BASE64]$3')
    .replace(/(^|[^A-Za-z0-9+/_-])([A-Za-z0-9+/]{80,}={0,2})($|[^A-Za-z0-9+/_=-])/g, '$1[REDACTED_BASE64]$3')
    .replace(/(^|[^\\\w])\\\\[^\\\s<>"']+\\[^\\\s<>"']+(?:\\[^\s<>"']*)?/g, '$1[REDACTED_LOCAL_PATH]')
    .replace(/\b[A-Za-z]:\\[^\s<>"']+/g, '[REDACTED_LOCAL_PATH]')
    .replace(/(^|[^A-Za-z0-9:])\/(?:private\/tmp|var\/folders|Users|Volumes|home|opt|tmp|root)\/[^\s<>"']+/g, '$1[REDACTED_LOCAL_PATH]')
    .replace(/<[^>]*>/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 500)
