...
export function buildLocalWikiActions(input: {
  ...
  const maxActions = Math.max(1, Math.min(60, input.maxActions || 30))
  ...
  for (const raw of input.rawFiles || []) {
    ...
    for (const section of sections) {
      ...
      if (!existingTopFolders.has(folder)) newFolders.add(folder)
      actions.push(buildWikiPage(section, folder))
      addedForRaw++
    }
