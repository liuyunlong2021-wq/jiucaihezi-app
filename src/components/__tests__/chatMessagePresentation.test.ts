...
test('ChatPanel maps OpenCode P0 commands to native UI actions instead of invalid server slash commands', () => {
  ...
  assert.match(chatPanel, /async function runSessionMenuCommand\(command: string\)/)
  assert.match(chatPanel, /await runVisibleSlashText\(`\/\$\{command\}`/)
  assert.match(chatPanel, /@click="runSessionMenuCommand\('compact'\)"/)
  assert.match(chatPanel, />压缩上下文</)
  assert.equal((chatPanel.match(/>压缩上下文</g) || []).length, 1)
  assert.doesNotMatch(chatPanel, /compactCurrentOpenCodeContext/)
  assert.doesNotMatch(chatPanel, /cp-clear-context-btn/)
  assert.match(chatPanel, /:disabled="!canCompactContext" @click="runSessionMenuCommand\('compact'\)"/)
...
test('OpenCode session menu actions keep local session state aligned', () => {
  ...
  assert.match(chatPanel, /if \(action === 'compact' && !canCompactContext\.value\) \{[\s\S]*setLocalCommandNotice\('当前没有可压缩的 OpenCode 上下文，或会话仍在执行\/加载中。'\)[\s\S]*showSessionCommandMenu\.value = false[\s\S]*showComposerCommandMenu\.value = false[\s\S]*return/)
  ...
  assert.match(chatPanel, /if \(!result\.ok\) \{[\s\S]*showSessionCommandMenu\.value = false[\s\S]*showComposerCommandMenu\.value = false[\s\S]*return[\s\S]*\}/)
...
test('OpenCode session status and abort use official visible phases', () => {
  ...
  assert.match(useChat, /setPhase\('cancelling', 'OpenCode 正在停止'\)/)
