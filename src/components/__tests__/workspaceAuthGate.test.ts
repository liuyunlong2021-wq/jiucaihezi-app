...
test('ChatPanel hides and bypasses skill and knowledge controls for non-members', () => {
  ...
  assert.equal(source.includes('isCloudLoggedIn'), true)
  assert.equal(source.includes('const isMember = computed'), true)
  assert.equal(source.includes('v-if="isMember && pendingInvoke"'), true)
  assert.equal(source.includes('function handleConfirmChain'), true)
  assert.equal(source.includes('if (!isMember.value) {\n    rejectChainInvoke()\n    return\n  }'), true)
  ...
  assert.equal(source.includes('if (!isMember.value) return undefined'), true)
