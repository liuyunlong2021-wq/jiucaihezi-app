  ...
  <div class="vrn-wrapper" @mouseenter="showHandleMenu = true" @mouseleave="showHandleMenu = false">
    <div class="vrn-card" :class="data.selected ? 'vrn-selected' : ''">
      <div class="vrn-header">
        <span v-if="!isEditingLabel" @dblclick="startEditLabel" class="vrn-header-label" title="双击编辑名称">{{ data.label || '视频结果' }}</span>
      ...
      <div class="vrn-body">
        ...
        <div v-else class="vrn-state vrn-empty">
          <span class="mso" style="font-size:32px;color:var(--ink3)">movie</span>
          <span>等待生成...</span>
        </div>
      </div>
...
.vrn-card { background: var(--surface-alt); border-radius: var(--radius); border: 1px solid var(--border); min-width: 320px; max-width: 400px; transition: all 0.2s; }
...
.vrn-empty { background: var(--surface); border: 2px dashed var(--border); }
