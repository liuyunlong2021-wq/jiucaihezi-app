...
<template>
  <NodeFrame :id="id" label="图片结果" icon="image" role="result" :selected="selected" :executable="false" @delete="$emit('delete',$event)" @resize-start="onResizeHandlePointerDown">
    ...
    <div class="v8-result gallery">
      <img v-if="url" :src="url" style="max-width:100%;border-radius:6px"/>
      <div v-else class="v8-gallery-placeholder">图片结果</div>
    </div>
  </NodeFrame>
</template>
<style scoped>
