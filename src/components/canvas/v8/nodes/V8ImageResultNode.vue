<template>
  <div class="irn-wrapper" @mouseenter="showHandleMenu = true" @mouseleave="showHandleMenu = false">
    <div class="irn-card" :class="data.selected ? 'irn-selected' : ''">
      ...
      <div class="irn-body" @drop.prevent="handleDrop" @dragover.prevent>
        <!-- Hidden file input -->
...
const handleSelect = (item: NodeHandleOperation) => {
  ...
  const n = canvasStore.addNodeWithData(item.type as any, { label: item.label, prompt: props.data?.url ? `参考图片: ${props.data.url}` : '' } as any, { x: (cn?.position?.x || 0) + 380, y: cn?.position?.y || 0 })
...
.irn-card { position: relative; background: var(--surface-alt); border-radius: var(--radius); border: 1px solid var(--border); min-width: 200px; max-width: 280px; transition: all 0.2s; }
