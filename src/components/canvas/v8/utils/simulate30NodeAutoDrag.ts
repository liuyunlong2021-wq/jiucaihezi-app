 ...
 *
 * 目标：
 * - 加载多样化 V8 节点（Text, LLM, Context Providers, MediaGen, Group, Loop 等）
...
export function createDiverse30NodeCanvas() {
  ...
  console.log('[V8 AutoDrag] 已加载 30 节点多样化重负载画布（含 Group/LLM/Context/Media 等）')
}

export async function runAutomaticDragSequence(durationMs = 6500) {
  console.log('[V8 AutoDrag] 开始自动拖拽序列...')

  const nodes = canvasStore.nodes
  if (nodes.length < 20) {
    console.warn('[V8 AutoDrag] 节点数量不足，建议先调用 loadHeavyCanvas')
    return
  }
  ...
  console.log(`[V8 AutoDrag] 自动拖拽序列完成，共执行约 ${dragCount} 次位置更新`)
}

export async function runFull30NodeAutoDragBenchmark() {
  console.log('%c[V8] === 启动完整 30 节点自动模拟拖拽基准 ===', 'color:#10b981; font-size:13px')
