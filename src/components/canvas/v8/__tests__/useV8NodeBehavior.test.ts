...
import { useV8NodeBehavior } from '../composables/useV8NodeBehavior.ts'

describe('useV8NodeBehavior (Phase 0)', () => {
  const fakeNode = {
    id: 'test-node',
  ...
  test('should expose isInteracting and resizingId (structure)', () => {
    const { isInteracting, resizingId } = useV8NodeBehavior(fakeNode)
  ...
  test('should provide onResizeHandlePointerDown handler (structure)', () => {
    const { onResizeHandlePointerDown } = useV8NodeBehavior(fakeNode, {
      onResizeEnd: () => {}
    })
