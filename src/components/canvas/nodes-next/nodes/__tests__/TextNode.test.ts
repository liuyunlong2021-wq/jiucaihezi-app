 ...
 *
 * Test-first approach: This file created before TextNode.vue implementation.
 *
 ...
 *   - Existing performanceBenchmark + simulateHeavyCanvas (nodes-next/utils)
 *   - TN-003 directly exercises the 30-node harness with TextNodes.
 *
 * When TextNode is integrated + canvas re-enabled, run:
...
describe('TextNode TDD (TN-001/002/003 + C-015)', () => {
  test('TN-001: at most one full Tiptap instance live', () => {
    ...
    // See TextNode.vue: singleEditorInstance, blurHandler, degradeToPreview()
  ...
  test('TN-002: degrade speed on blur (Tiptap destroy < 80ms)', () => {
    ...
    // Performance harness in nodes-next/utils/performanceBenchmark.ts can time this.
  ...
  test('TN-003: 30-node benchmark with TextNodes (Jank < 200ms)', () => {
    ...
    //   import { simulateHeavyCanvas } from '@/components/canvas/nodes-next/utils/simulateHeavyCanvas'
    //   // 5 of 30 nodes are TextNode that were previously expanded
  ...
  test('C-015: LLM → Text edge for 5-node template (prompt-flow)', () => {
    // Given: LlmNode with right-text source Handle
    // When: connect to TextNode left-prompt target Handle
  ...
  test('TextNode uses NodeFrame + role=input + Handles for prompt-flow', () => {
...
/**
 * To "let it pass":
 * 1. Implement TextNode.vue satisfying the comments above.
 * 2. After registration in CanvasWorkspace, manually exercise TN-001/002 in browser.
 * 3. Run 30-node simulator with TextNodes: see nodes-next/examples/runBenchmark.ts or console runV8Phase0Benchmark()
