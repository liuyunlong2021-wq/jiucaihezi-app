...
import { useNodeResize } from '../composables/useNodeResize.ts'
...
describe('useNodeResize (Phase 0)', () => {
  ...
  test('should return handlePointerDown function and integrate with freeze (basic structure test for env)', () => {
    const onResizeEnd = () => {}
    const { handlePointerDown, resizingId } = useNodeResize({ onResizeEnd })
