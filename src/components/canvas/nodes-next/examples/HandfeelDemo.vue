...
import { 
  useNodeBehavior, 
  freezeManager 
} from '@/components/canvas/nodes-next'
...
} = useNodeBehavior(nodeData as any, {
  onResizeEnd: (id, w, h) => {
    nodeData.width = w
    nodeData.height = h
    console.log('[Demo] Resize 完成:', w, h)
  }
})
...
watchEffect(() => {
  if (resizingId.value) {
    // 在真实 resize 中，useNodeResize 会通过 RAF 更新 DOM
    // 这里我们模拟显示
  }
})
