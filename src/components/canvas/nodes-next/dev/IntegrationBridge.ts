 ...
 * Phase 0 收尾 - 真实环境集成桥梁
 *
 * 目标：提供安全、非破坏性的方式把 手感基础设施接入现有 CanvasWorkspace。
 * 当前为开发/验证用途，最终会在后续 Phase 进行完整替换。
 */

import { activatePhase0 } from '../activate'
import { useNodeBehavior } from '../composables/useNodeBehavior'
...
export function enableV8HandfeelInDev() {
  if (import.meta.env.DEV) {
    activatePhase0()
    console.log('%c[Phase 0] 手感系统已在开发模式激活', 'color: #10b981')
  }
}
...
export function shouldUseV8Node(): boolean {
  ...
  // 开发期开关：localStorage.setItem('useNodes', 'true')
  return localStorage.getItem('useNodes') === 'true'
}
