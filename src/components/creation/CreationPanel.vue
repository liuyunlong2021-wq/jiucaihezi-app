...
import { RH_TASK_LABELS, type CreationTask } from '@/data/creationModels'
import {
...
const totalTaskPages = computed(() =>
  Math.max(1, Math.ceil(creationTasksTotal.value / taskPageSize.value)),
)
...
const offTaskSettled = onEvent('media-task-settled', (payload: any) => {
  if (payload.source === 'creation') {
    ...
    if (!cpState.generating) {
      const errMsg =
        payload.status === 'failed' ? `\u274C 生成失败: ${payload.errorMsg || '请重试'}` : ''
  ...
  nextMarkerNumber =
    Math.max(
      ...
      ...app.tree.children
        ...
        .map((marker: any) => Number(marker.children?.[1]?.text) || 0),
    ) + 1
}
...
function releaseCanvasAssetRuntimeUrl(assetId: string) {
  ...
  if (
    !resource ||
    [...canvasAssetRuntimeResources.values()].some(
      candidate => candidate.owner === resource.owner && candidate.path === resource.path,
    )
  )
    return
...
function cancelCanvasInteraction() {
  ...
  if (ids) {
    ids.forEach((id: any) => app!.off_(id))
...
function isCurrentCanvasMediaRequest(request: CanvasMediaOwnership): boolean {
  return (
    request.loadToken === canvasLoadToken &&
...
function selectCanvasReferences(nodes: any[]) {
  ...
  const selected = app.editor.list.filter(node =>
    Boolean(canvasStore.assets[String((node as any).id)]),
  )
  ...
  return new Promise(resolve => {
    ...
    probe.onload = () => {
      const scale = Math.min(
        1,
        CANVAS_MEDIA_WIDTH / probe.naturalWidth,
        CANVAS_MEDIA_HEIGHT / probe.naturalHeight,
      )
      resolve({
...
async function addMediaToCanvas(
  ...
  const size =
    kind === 'image'
      ...
      : kind === 'video'
        ? { width: CANVAS_MEDIA_WIDTH, height: 180 }
  ...
  if (kind === 'video') {
    ...
    void hydrateVideoReferenceNode(card, filePath, layer.id, owner, () =>
    ...
    app.tree.add(card)
    selectCanvasReferences([card])
    void hydrateAudioReferenceNode(card, filePath, layer.id, owner, () =>
      isCurrentCanvasMediaRequest(request),
    )
  } else {
    const img = new Image({
...
function relinkCanvasAsset(filePath: string, kind: CanvasMediaKind, owner: string): boolean {
  ...
  void restoreCanvasScene(document, path, owner, () =>
    isCurrentCanvasTarget(loadToken, owner, path),
...
function addFileTreeMediaToCanvas(payload: any) {
  ...
  void addMediaToCanvas(
    ...
    label,
    '',
    captureCanvasMediaRequest(filePath, kind, 'import', label, '', {
      owner: projectId,
      loadToken: canvasLoadToken,
    }),
  )
}
...
function reconcileCurrentCanvasMediaEntry(change: ProjectResourceChangeEntry): boolean {
  ...
  const matching = Object.values(canvasStore.assets).filter(
    asset => asset.resource.path === resource.path,
  )
  ...
  if (change.type === 'renamed') {
    for (const asset of matching) {
      asset.resource = {
        ...
        ...(change.resource.id ? { id: change.resource.id } : {}),
      }
      asset.missing = false
    }
...
async function addCanvasFiles(files: Iterable<File>) {
  ...
  for (const file of files) {
    const kind: CanvasMediaKind | null = file.type.startsWith('image/')
      ? 'image'
...
function isCurrentCanvasTarget(loadToken: number, owner: string, path: string): boolean {
  return (
    isCurrentCanvasLoad(loadToken, owner) &&
    canvasOwner.value === owner &&
    canvasStore.canvasPath === path
  )
}
...
async function restoreCanvasScene(
  ...
  for (const node of document.scene) {
    ...
    if (asset?.missing) {
      app.tree.add(
        createMissingMediaNode(
          asset.id,
          Number((node as any).x || 0),
          Number((node as any).y || 0),
          mediaDisplayName(asset.resource.path),
          node,
        ),
      )
      continue
    ...
    if (asset?.kind === 'video') {
      const card = createVideoReferenceNode(
        asset.id,
        Number((node as any).x || 0),
        Number((node as any).y || 0),
        videoDisplayLabel(asset.resource.path),
        node,
      )
      app.tree.add(card)
      void hydrateVideoReferenceNode(
        card,
    ...
    if (asset?.kind === 'audio') {
      const card = createAudioReferenceNode(
        asset.id,
        Number((node as any).x || 0),
        Number((node as any).y || 0),
        mediaDisplayName(asset.resource.path),
      ...
      app.tree.add(card)
      void hydrateAudioReferenceNode(
        card,
        isTauriRuntime() ? `${projectDir}/${asset.resource.path}` : asset.resource.path,
        asset.id,
        owner,
...
function reportCanvasRestoreFailure(error: unknown) {
  ...
  cpState.progressText = `画布无法打开: ${reason}。原文件未被覆盖`
}

function clearCanvasRestoreFailure() {
  if (cpState.progressText.startsWith('画布无法打开')) cpState.progressText = ''
}
...
async function openCanvas(path: string, owner = selectedCanvasOwner(), keepGate = false) {
  ...
  const staleGate =
    !keepGate && currentGate && (currentGate.owner !== owner || currentGate.path !== path)
      ? currentGate
      : undefined
  ...
  try {
    ...
    if (result.status !== 'ready') throw new Error('画布文件不存在或已被移除')
    canvasReady = false
    await restoreCanvasScene(result.document, path, owner, () =>
      isCurrentCanvasLoad(loadToken, owner),
    )
    if (!isCurrentCanvasLoad(loadToken, owner)) return
    canvasReady = true
    clearCanvasRestoreFailure()
...
async function createAndOpenCanvas(owner = selectedCanvasOwner(), keepGate = false) {
  ...
  try {
    ...
    if (!isCurrentCanvasLoad(loadToken, owner)) return
    canvasReady = false
    await restoreCanvasScene(document, file.path, owner, () =>
      isCurrentCanvasLoad(loadToken, owner),
    )
    if (!isCurrentCanvasLoad(loadToken, owner)) return
    canvasReady = true
...
const offCanvasLifecycleFailed = onEvent('canvas:lifecycle-failed', (payload: any) => {
  ...
  if (
    canvasStore.canvasPath === gate.path &&
    canvasOwner.value === gate.owner &&
    selectedCanvasOwner() === gate.owner
  ) {
    canvasReady = true
    setCanvasRestoring(false)
  }
...
const offCanvasOpen = onEvent('canvas:open', (payload: any) => {
  if (payload?.path)
    void openCanvas(payload.path).catch(error => {
      cpState.progressText = `打开画布失败: ${String(error.message || error)}`
    })
})
...
const offCanvasBeforeTaskWrite = onEvent('canvas:before-task-write', async (payload: any) => {
  ...
  if (
    ...
    return
  if (activeCanvasGate || !canvasReady || canvasRestoring)
    throw new Error('画布正在恢复，请稍后重试')
...
const offCanvasTaskResult = onEvent('canvas:task-result', async (payload: any) => {
  ...
  const release =
    typeof payload?.release === 'function' ? (payload.release as () => void) : undefined
  if (
    !payload?.document ||
    path !== canvasStore.canvasPath ||
    owner !== canvasOwner.value ||
  ...
  for (const child of app.tree.children as any[]) {
    if (child.name === 'canvas-video-reference' || child.name === 'canvas-audio-card') {
      for (const item of child.children || []) {
        if (item.name === 'video-frame' || item.name === 'audio-frame')
          item.set({ fill: getCanvasFill(), stroke: getCanvasFrame() })
...
function createVideoReferenceNode(
  id: string,
  x: number,
  y: number,
  label: string,
  saved?: CanvasSceneNode,
) {
...
function createAudioReferenceNode(
  id: string,
  x: number,
  y: number,
  label: string,
  ...
  card.addMany(
    ...
    new LeaferText({
      ...
      y: 20,
      width: width - 76,
      text: missing ? `素材已缺失 · ${label}` : label,
      fill: getCanvasText(),
      fontSize: 13,
      textWrap: 'none',
      textOverflow: 'ellipsis',
    ...
    new LeaferText({
      ...
      y: 48,
      width: width - 76,
      text: missing ? '请重新关联或移除' : formatVideoDuration(duration) || '音频',
      fill: getCanvasText(),
      fontSize: 12,
    }),
  )
...
function createMissingMediaNode(
  id: string,
  x: number,
  y: number,
  label: string,
...
async function toggleCanvasAudio(
  filePath: string,
  assetId: string,
  owner: string,
  canContinue: CanvasLoadGuard,
) {
  if (!canContinue()) return
...
async function hydrateVideoReferenceNode(
  ...
  try {
    ...
    if (asset) {
      ...
      if (label)
        label.text = `${videoDisplayLabel(filePath)}${formatVideoDuration(preview.duration) ? ` · ${formatVideoDuration(preview.duration)}` : ''}`
    }
    scheduleCanvasSave()
  } catch {}
}
...
function canvasTool(action: string) {
  ...
  switch (action) {
    ...
    case 'draw': {
      ...
      const ids = [
        ...(isText
          ? [app.on_(PointerEvent.DOWN, onTextDown)]
          : isNumber
            ? [app.on_(PointerEvent.DOWN, onNumberDown)]
            : [
    ...
    case 'zoomIn':
      setCanvasViewportScale(Number(app.zoomLayer.scale || 1) * 1.3)
      break
    case 'zoomOut':
      setCanvasViewportScale(Number(app.zoomLayer.scale || 1) / 1.3)
      break
    case 'group':
      if (app.editor?.list?.length && app.editor.list.length > 1) {
        app.editor.group()
        saveCanvasHistory()
      }
      break
    ...
    case 'copy': {
      ...
      for (const el of app.editor.list) {
        try {
          clipboard.push((el as any).clone())
        } catch {
          /* 某些元素可能不支持 clone */
        }
      }
...
onMounted(() => {
  ...
  const onKeyDown = (e: KeyboardEvent) => {
    ...
    if (e.key === 'Escape' && drawMode.value) {
      ...
      !e.altKey &&
      !e.shiftKey &&
      !app.editor?.innerEditing &&
      ['v', 'a', 't', 'b', 'n'].includes(e.key.toLowerCase())
    ) {
      ...
      if (e.key.toLowerCase() === 'v') {
        ...
        e.preventDefault()
        activateDrawTool(toolKeys[e.key.toLowerCase()])
      }
...
const canSend = computed(
  () =>
    Boolean(currentCreationSpec.value) &&
    currentModelAvailability.value?.status !== 'disabled' &&
    Boolean(cpState.prompt?.trim()),
)
</script>
...
<template>
  <div class="cp" data-panel="creation">
    <div class="cp-toolbar">
      ...
      <button
        class="cp-toolbar-link"
        @click="openExternal('https://dazi.studio/')"
        title="打开提示词参考"
      >
    ...
    <div
      ...
      data-project-drop-target="canvas"
      :class="{
        'cp-canvas-dragover': canvasDragOver,
        'cp-canvas-interaction-blocked': canvasInteractionBlocked,
      }"
    >
      <div
        ref="canvasContainer"
        class="cp-canvas-container"
        tabindex="0"
      ...
      <div v-if="selectedReferenceAssets.length" class="cp-reference-state">
        ...
        <span v-if="unsupportedReferenceSummary" class="cp-reference-ignored"
          >当前模型不使用 {{ unsupportedReferenceSummary }}</span
        >
      </div>
      <!-- 进度浮层（生成时叠在画布左下角） -->
      ...
      <div class="cp-canvas-toolbar">
        <input
          ref="canvasImportInput"
          class="cp-canvas-import"
          type="file"
          multiple
        ...
        <button
          title="画箭头 A"
          :class="{ active: drawMode && drawType === 'arrow' }"
          @click="activateDrawTool('arrow')"
        >
          <JcIcon name="arrow_forward" />
        </button>
        <button
          title="写文字 T"
          :class="{ active: drawMode && drawType === 'text' }"
          @click="activateDrawTool('text')"
        >
          <JcIcon name="title" />
        </button>
        <button
        ...
        <div v-if="showCanvasMore" class="cp-canvas-more" @click.stop>
          ...
          <button title="下移一层" @click="runCanvasMore('layerDown')">
            <JcIcon name="arrow_downward" />
          </button>
          <button title="置顶" @click="runCanvasMore('toFront')">
            <JcIcon name="vertical_align_top" />
          </button>
          <button title="置底" @click="runCanvasMore('toBack')">
          ...
          <button title="解锁" @click="runCanvasMore('unlock')">
            <JcIcon name="toggle_off" />
          </button>
          <span>变换</span>
          <button title="左转 90 度" @click="runCanvasMore('rotateLeft')">
            <JcIcon name="rotate_left" />
          </button>
          <button title="右转 90 度" @click="runCanvasMore('rotateRight')">
            <JcIcon name="rotate_right" />
          </button>
          <button title="水平翻转" @click="runCanvasMore('flipHorizontal')">
            <JcIcon name="flip" />
          </button>
          <button title="垂直翻转" @click="runCanvasMore('flipVertical')">
            <JcIcon name="flip" class="cp-flip-vertical" />
          </button>
          <button title="向左倾斜" @click="runCanvasMore('skewLeft')">
            <JcIcon name="transform" />
          </button>
      ...
      <Teleport to="body">
        ...
        >
          ...
          <button
            ...
            "
          >
            <JcIcon name="format_list_numbered" />编号标注
          </button>
          <button
            @click="
              canvasTool('select');
      ...
      <div v-if="showTaskHistory" class="cp-history-overlay" @click.self="showTaskHistory = false">
        <div class="cp-history-modal">
          <div class="cp-history-head">
            <h3>任务历史</h3>
            <button class="cp-history-close" @click="showTaskHistory = false">
              <JcIcon name="close" />
            </button>
          </div>
          ...
          <div v-else class="cp-task-list">
            ...
            >
              <div class="cp-task-head">
                ...
                <span class="cp-task-time">{{
                  formatTaskTime(task.completedAt || task.createdAt)
                }}</span>
              </div>
              ...
              <div
                ...
                class="cp-task-path"
                :class="{ remote: !task.assetUri && !task.projectPath }"
              >
                {{ taskPath(task) }}
              </div>
              <div
        ...
        <div
          v-if="
            ...
            ((field.key !== 'customWidth' && field.key !== 'customHight') ||
              cpState.ar === 'custom')
          "
          class="cp-island cp-generic-field"
        >
      ...
      <div class="cp-composer-row">
        <div class="cp-prompt-wrap">
          ...
          <div v-if="showTagsInput" class="cp-suno-row">
            <input
              v-model="cpState.tags"
              placeholder="风格标签 (如: pop, rock, edm)"
              class="cp-suno-input"
              @blur="saveCpState()"
            />
          </div>
          ...
          <div v-if="showStartEndTimeInput" class="cp-inline-fields">
            <input
              v-model="cpState.startTime"
              placeholder="开始时间 0:00"
              class="cp-suno-input"
              @blur="saveCpState()"
            />
            <input
              v-model="cpState.endTime"
              placeholder="结束时间 0:11"
              class="cp-suno-input"
              @blur="saveCpState()"
            />
          </div>
          <div v-if="showRefTextInput" class="cp-suno-row">
            <textarea
              v-model="cpState.refText"
              rows="2"
              placeholder="参考音频文字"
              class="cp-aux-textarea"
              @blur="saveCpState()"
            />
          </div>
          <div v-if="showTextInput" class="cp-suno-row">
            <textarea
              v-model="cpState.text"
              rows="2"
              :placeholder="cpState.modelKey === 'rh-aiapp' ? '输出文字/文稿' : '输出文字/文稿'"
              class="cp-aux-textarea"
              @blur="saveCpState()"
            />
          </div>
          <div v-if="showVoicePromptInput" class="cp-suno-row">
            <textarea
              v-model="cpState.voicePrompt"
              rows="2"
              placeholder="人设 + 音色特征 + 风格 + 情感 + 节奏"
              class="cp-aux-textarea"
              @blur="saveCpState()"
            />
          </div>
...
.cp-canvas-toolbar .cp-canvas-more button {
  width: 28px;
  height: 28px;
}
.cp-flip-vertical {
...
.cp-task-item.pending {
  border-left: 3px solid var(--olive);
}
.cp-task-item.success {
...
.cp-task-pagination button:hover:not(:disabled) {
  border-color: var(--olive);
  color: var(--olive-dark);
}
.cp-task-pagination button:disabled {
  opacity: 0.35;
  cursor: default;
}
...
.cp-generic-media-row {
  display: flex;
  align-items: center;
  gap: 4px;
}
.cp-generic-media-row .cp-generic-input {
  flex: 1;
  min-width: 0;
}
.cp-pick-media-btn {
...
.cp-file-thumb:hover .cp-thumb-remove {
  opacity: 1;
}
.cp-clear-files {
  display: inline-flex;
