...
function isWikiCreationRequest(text: string): boolean {
  return /(?:创建|新建|建立|搭建).*?(?:wiki|知识库)|(?:wiki|知识库).*?(?:创建|新建|建立|搭建)/.test(text.toLowerCase())
}
...
export function selectMemoryTools(
  ...
  const explicitWiki =
    /wiki|知识库|角色设定|查询.*设定|查.*进度|巡检|断链|规划.*目录|填充.*wiki|写入.*wiki|修正.*wiki/.test(
      text,
    )
  const wikiCreation = isWikiCreationRequest(text)
  const wikiManagement =
    wikiCreation || /巡检|断链|规划.*目录|填充.*wiki|修正.*wiki|关系图|审计|脚手架/.test(text)
  const explicitFile = !wikiCreation && (
    /写入|更新|保存|创建文件|修改文件|读取文件|查看文件|本地文件|文件夹/.test(text) ||
  ...
  if (explicitWiki && !wikiCreation) allowed.add(WIKI_CONTEXT_TOOL_DEFINITION.function.name)
  if (wikiManagement) allowed.add('wiki')
  if (explicitFile)
    for (const name of ['read', 'glob', 'grep', 'write', 'edit', 'mkdir']) allowed.add(name)
  if (explicitTerminal) allowed.add('terminal')
  if (explicitMcp)
    for (const tool of tools)
      if (tool.function?.name?.startsWith('mcp__')) allowed.add(tool.function.name)
  ...
  return tools.filter(
    tool =>
      ...
      !(
        explicitWiki &&
        !wikiManagement &&
        ['wiki_search', 'glob', 'grep'].includes(tool.function?.name)
      ),
  )
}
...
export async function runMemoryChat(input: MemoryChatInput): Promise<string> {
  ...
  if (!memoryMode) {
    const result = await runDirectChatCompletion({
      messages,
      tools: [WIKI_CONTEXT_TOOL_DEFINITION],
      sendChatCompletion,
      signal: input.signal,
      onText: input.onText,
      onToolEvent(event) {
  ...
  const memoryToolDefinitions = selectMemoryTools(
    ...
    Boolean(input.attachments?.some(attachment => attachment.kind === 'file' && attachment.readablePath && !attachment.textContent)),
  )
  const wikiMutationIntent =
    /写入|添加|更新|修正|补充|纳入|合并|替换|扩展|填充|创建|新建|建立|搭建|重建|重构|整理|迁移/.test(latestUserText) &&
    /wiki|知识库|规则|规范/.test(latestUserText.toLowerCase())
  const wikiBatchCreationIntent = isWikiCreationRequest(latestUserText)
  ...
  const maxMemorySteps = hasExtendedTool ? 5 : wikiMutationIntent && !wikiBatchCreationIntent ? 5 : 3
  const result = await runDirectChatCompletion({
    messages,
    ...
    finalizeAtModelRequestLimit:
      hasExtendedTool && memoryToolDefinitions.some(tool => tool.function.name.startsWith('mcp__')),
    stopAfterSuccessfulToolNames: ['write', 'edit'],
    stopAfterSuccessfulToolCall: call => {
      if (!wikiBatchCreationIntent || call.function.name !== 'wiki') return false
      const args = parseArguments(call.function.arguments)
      return args.action === 'scaffold' && Boolean(args.plan)
    },
