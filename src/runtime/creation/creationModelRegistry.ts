...
export const CREATION_MODEL_REGISTRY: CreationModelSpec[] = [
  ...
  directVideo({
    ...
    assetFlow: 'newapi-upload',
    contractStatus: 'verified',
    // 方向写在分辨率后缀里（竖/横/(1:1)），这两个模型没有比例参数：面板不展示比例，请求也不带。
    ratios: [],
    resolutions: ['480p竖', '768p竖', '480p横', '768p横'],
    duration: { min: 1, max: 15 },
    files: {
      images: { min: 1, max: 9, maxBytes: 20 * 1024 * 1024 },
      audios: { min: 0, max: 3, maxBytes: 20 * 1024 * 1024 },
    },
    fields: promptFields([
      ...
      { key: 'images', label: '参考图 (1-9张)', kind: 'images' },
  ...
  directVideo({
    ...
    fields: promptFields([
      { key: 'resolution', label: '分辨率', kind: 'select', defaultValue: '768p竖', options: options(['480p竖', '768p竖', '480p横', '768p横', '480p(1:1)', '768p(1:1)']) },
  ...
  directImage({
    id: 'newapi/zx/mj_fast_imagine',
    model: 'mj_fast_imagine',
    label: 'Midjourney Fast Imagine · ZX',
    price: 'ZX 实时计费',
    upstreamFamily: 'zx',
    apiStyle: 'newapi-image-task',
    mode: 'text-to-image',
    endpoint: '/v1/videos',
    pollKind: 'newapi-task',
    assetFlow: 'none',
    ...
    files: { images: { min: 0, max: 8 } },
    fields: promptFields([{ key: 'images', label: '参考图', kind: 'images' }]),
    notes: ['docs/wiki/运维/ZX API Midjourney 使用文档'],
    contractIssues: ['仅接 ZX Fast Imagine；尚未接入 Relax 与按钮后续操作。'],
  }),
...
export function creationModelFamily(spec: Pick<CreationModelSpec, 'id' | 'model' | 'task'>): string {
  ...
  if (id.includes('grok-image') || id.includes('grok-imagine-image') || (id.includes('grok-') && id.includes('video'))) return spec.task === 'video' ? 'Grok Video' : 'Grok Image'
