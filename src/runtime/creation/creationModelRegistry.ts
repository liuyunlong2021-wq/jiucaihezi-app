...
export const CREATION_MODEL_REGISTRY: CreationModelSpec[] = [
  baseSpec({
    ...
    aliases: ['gpt-image-2', 'gpt2.0'],
    notes: ['docs/notes/T8gpt2.md'],
    ratios: ['1:1', '2:3', '3:2', '4:5', '5:4', '4:3', '3:4', '16:9', '9:16', '21:9'],
    resolutions: ['1k', '2k', '4k'],
  }),
  directImage({ id: 'newapi/direct/gpt2.0', model: 'gpt2.0', label: 'GPT2.0 · NewAPI 直连', price: 0.15, upstreamFamily: 'openai-compatible', notes: ['NewAPI alias'], aliases: ['gpt2.0'] }),
  ...
  // ── 特朗普/WorldRouter async 图片 (POST /v1/images/generation_tasks → GET /v1/images/generation_tasks/{task_id}) ──
  // 文档: docs/notes/特朗普-GPTImage2.md
  baseSpec({
    id: 'newapi/trump/gpt-image-2',
    model: 'gpt-image-2',
    label: 'GPT Image 2 · 特朗普',
    task: 'image',
    source: 'newapi-direct',
    route: 'newapi-direct',
    upstreamFamily: 'trump',
    apiStyle: 'trump-image-task',
    mode: 'text-to-image',
    contractStatus: 'partial',
    price: 0.15,
    endpoint: '/v1/images/generation_tasks',
    pollKind: 'newapi-task',
    assetFlow: 'none',
    resultExtractor: 'openai-image',
    files: { images: { min: 0, max: 8 } },
    fields: promptFields([
      { key: 'size', label: '尺寸', kind: 'select', defaultValue: '1024x1024', options: options(GPT_IMAGE_SIZES) },
      { key: 'output_format', label: '输出格式', kind: 'select', defaultValue: 'png', options: options(['png', 'jpeg', 'webp']) },
      { key: 'quality', label: '质量', kind: 'select', defaultValue: 'low', options: options(['low', 'medium', 'high']) },
      { key: 'images', label: '参考图', kind: 'images' },
    ]),
    notes: ['docs/notes/特朗普-GPTImage2.md'],
    ratios: GPT_IMAGE_SIZES.map(s => s),
    contractIssues: ['异步轮询字段需实测确认。'],
  }),
  baseSpec({
    id: 'newapi/trump/gemini-3.1-flash-image-preview',
    model: 'gemini-3.1-flash-image-preview',
    label: 'Gemini 3.1 Flash Image · 特朗普',
    task: 'image',
    source: 'newapi-direct',
    route: 'newapi-direct',
    upstreamFamily: 'trump',
    ...
    fields: promptFields(),
    notes: ['docs/notes/特朗普-GPTImage2.md'],
    contractIssues: ['Gemini 忽略 size 参数，模型自行决定输出分辨率。异步轮询字段需实测确认。'],
  }),
  baseSpec({
    id: 'newapi/trump/gemini-3-pro-image-preview',
    model: 'gemini-3-pro-image-preview',
    label: 'Gemini 3 Pro Image · 特朗普',
    task: 'image',
    source: 'newapi-direct',
    route: 'newapi-direct',
    upstreamFamily: 'trump',
    ...
    fields: promptFields(),
    notes: ['docs/notes/特朗普-GPTImage2.md'],
    contractIssues: ['Gemini 忽略 size 参数。异步轮询字段需实测确认。'],
  }),
  baseSpec({
    id: 'newapi/trump/gemini-2.5-flash-image',
    model: 'gemini-2.5-flash-image',
    label: 'Gemini 2.5 Flash Image · 特朗普',
    task: 'image',
    source: 'newapi-direct',
    route: 'newapi-direct',
    upstreamFamily: 'trump',
  ...
  runninghubStandard({ id: 'runninghub/api/rh-image-v2', model: 'rh-image-v2', label: '全能图片 V2 · RunningHub', task: 'image', mode: 'text-to-image', price: 0.3, notes: ['RH capabilities'] }),
  ...
  runninghubStandard({ id: 'runninghub/api/rh-grok-video-edit', model: 'rh-grok-video-edit', label: 'Grok Video 视频编辑 · RunningHub', task: 'video', mode: 'video-edit', price: 0.08, notes: ['docs/notes/runninghub-grok-video-3文档.md'], files: { videos: { min: 1, max: 1 } }, contractStatus: 'partial' }),
  ...
  runninghubStandard({ id: 'runninghub/api/rh-seedance2-multimodal-video', model: 'rh-seedance2-multimodal-video', label: 'Seedance 2.0 全能参考 · RunningHub', task: 'video', mode: 'workflow', price: 1.5, notes: ['docs/notes/runninghub-seedance文档.md'], files: { images: { min: 0, max: 9 }, videos: { min: 0, max: 1 }, audios: { min: 0, max: 1 } }, duration: { min: 4, max: 15 }, aliases: ['Seedance 2.0 文生/图生/全能生视频'] }),
  runninghubStandard({ id: 'runninghub/api/rh-seedance2', model: 'rh-seedance2', label: 'Seedance 2.0 聚合 · RunningHub', task: 'video', mode: 'workflow', price: 1.5, notes: ['docs/notes/runninghub-seedance文档.md'], contractStatus: 'partial', aliases: ['Seedance 2.0 文生/图生/全能生视频'] }),
  ...
  runninghubStandard({ id: 'runninghub/api/rh-voice-design', model: 'rh-voice-design', label: '声音设计 · RunningHub', task: 'audio', mode: 'voice-design', price: 0.15, notes: ['RH capabilities'], contractStatus: 'unknown' }),
  ...
  runninghubStandard({ id: 'runninghub/aiapp/rh-aiapp-director', model: 'rh-aiapp-director', label: '我是导演 · RunningHub 工作流', task: 'video', mode: 'workflow', price: 0.5, apiStyle: 'rh-aiapp', contractStatus: 'partial', notes: ['docs/notes/runninghub 5个工作流模型参数.md'], files: { images: { min: 1, max: 1 }, videos: { min: 1, max: 1 } }, fields: promptFields([{ key: 'image', label: '你想让谁演', kind: 'image', required: true }, { key: 'video', label: '你想让她/他演啥', kind: 'video', required: true }, { key: 'text', label: '动作说明', kind: 'textarea', required: true }, { key: 'width', label: '宽', kind: 'number', defaultValue: 480, min: 1, step: 1 }, { key: 'height', label: '高', kind: 'number', defaultValue: 832, min: 1, step: 1 }]) }),
  ...
  runninghubStandard({ id: 'runninghub/aiapp/rh-digital-human-fast', model: 'rh-digital-human-fast', label: '极速数字人 · RunningHub', task: 'digital-human', mode: 'digital-human', price: 0.15, apiStyle: 'rh-aiapp', contractStatus: 'partial', notes: ['docs/notes/runninghub 5个工作流模型参数.md'], files: { images: { min: 1, max: 1 }, audios: { min: 1, max: 1 } }, fields: [{ key: 'image', label: '人物照片', kind: 'image', required: true }, { key: 'audio', label: '驱动音频', kind: 'audio', required: true }, { key: 'value', label: '画面值', kind: 'number', required: true, defaultValue: 832, min: 16, step: 16 }], aliases: ['rh-digital-human-fast'] }),
  runninghubStandard({ id: 'runninghub/aiapp/rh-digital-human', model: 'rh-digital-human', label: '数字人 · RunningHub', task: 'digital-human', mode: 'digital-human', price: 0.2, apiStyle: 'rh-aiapp', contractStatus: 'partial', notes: ['docs/notes/runninghub 5个工作流模型参数.md'], files: { images: { min: 1, max: 1 }, audios: { min: 1, max: 1 } }, fields: promptFields([{ key: 'text', label: '台词', kind: 'textarea', required: true }, { key: 'image', label: '上传首帧图', kind: 'image', required: true }, { key: 'audio', label: '上传参考音频', kind: 'audio', required: true }, { key: 'width', label: '宽', kind: 'number', defaultValue: 540, min: 1, step: 1 }, { key: 'height', label: '高', kind: 'number', defaultValue: 960, min: 1, step: 1 }]), aliases: ['rh-digital-human'] }),
]
