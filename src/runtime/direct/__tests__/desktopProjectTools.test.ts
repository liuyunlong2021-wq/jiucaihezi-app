...
test('creative tool definitions append connected MCP tools without changing core tools', () => {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      ['skill', 'wiki', 'read', 'glob', 'grep', 'write', 'edit', 'mkdir', 'move', 'delete', 'export_markdown_png', 'create_document', 'create_html', 'export_markdown_slides', 'create_3d_scene', 'terminal', 'mcp__docs__lookup'],
    )
    assert.match(
