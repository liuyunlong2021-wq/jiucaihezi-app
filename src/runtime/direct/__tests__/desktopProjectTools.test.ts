...
test('desktop project tools do not write a rendered artifact after cancellation', async () => {
  ...
  const execute = createDesktopProjectToolExecutor({
    ...
    invoke: async command => {
      if (command === 'save_generated_file') writes += 1
      return { path: '/fixture/.raw/jc-media/图片/取消.png' }
    },
  })
