...
test('creative tool definitions append connected MCP tools without changing core tools', () => {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      ['skill', 'wiki', 'read', 'glob', 'grep', 'write', 'edit', 'mkdir', 'move', 'delete', 'export_markdown_png', 'create_document', 'create_html', 'create_3d_scene', 'terminal', 'mcp__docs__lookup'],
    )
    assert.match(
...
test('desktop memory artifact tools write generated bytes inside project materials', async () => {
  ...
  assert.equal(calls.filter(call => call.command === 'save_generated_file').length, 4)
